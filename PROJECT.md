...
- **桌面**：当前本地交付为 Windows **26.9.1802**，官网仍为 **26.9.18**（SQLite 至 `0030`）。在此前 26.9.17 的项目画板、多线程、身份引导、素材仅画板可见、分层编辑、Mac 共用更新、本地分类代理/GPU 与标签恢复基础上，合并工作区既有改动及设置内自动更新；采用用户确认的整包发布范围。老版本仍需先手动安装一次 26.9.18，之后由设置检查后续新版。
