...
**自动更新：** Windows x64 已发布 **26.9.1803**，含启动时检查更新、设计师路线开放范围及应用内账号升级入口；M 系列 Mac 线上为 **26.9.1901**（2026-09-20 确认清单已替换上线，含上述新增客户端功能与本地分类防误标），**26.9.2002 双架构（aarch64 + Intel 首发）已本机构建待上传发布**——向量匹配判官、Jev 云端校验、画板/文本卡增强、钥匙串弹窗修复，分发自此迁移腾讯 COS+CDN。官网检查、完整包下载和验签已通过，Intel、Mac 公证及双端原生升级仍待验收。发布/回滚和双端交接见 [桌面自动更新](dev-doc/DESKTOP-UPDATES.md)。
