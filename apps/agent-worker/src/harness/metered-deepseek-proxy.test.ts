...
test("metered DeepSeek proxy close waits for an accepted request to persist usage", async () => {
  const control = new FakeControl();
  ...
  const request = proxyRequest(proxy.childEnvironment()).catch(() => undefined);
  await upstreamStarted;
  let closed = false;
  const closing = proxy.close().then(() => { closed = true; });
  await new Promise<void>((resolve) => setTimeout(() => resolve(), 10));
  equal(closed, false);
  equal(control.usage.size, 0);
