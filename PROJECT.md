...
**9 月 18 日正式发布：** 用户确认将工作区原有改动与自动更新一起发布，官网现提供 `Bowerbird_26.9.18_x64-setup.exe`（83,178,026 bytes，SHA-256 `f7da5237d1d8d75cab5363769c9c3f76795fe0f725525e0046a74abd9ee5a781`）。HTTPS 更新入口、禁止缓存的版本清单、公开签名、完整安装包哈希、首页实际下载按钮及官网/后台健康检查通过；从公网下载的安装包再次使用内置公钥验签并验证篡改拒绝。官网当前目录 `/opt/bowerbird/website-releases/20260918-app-updater-v2`，回滚保留 `/opt/bowerbird/website-releases/20260914-code-admin`，旧安装包继续可下载。未执行真实应用升级，不宣称原生安装/重启/登录全链路已验收；源码已纳入本次统一存档提交。发布、回滚脚本和公网验收证据在 `.tmp/app-updater-release/`。
