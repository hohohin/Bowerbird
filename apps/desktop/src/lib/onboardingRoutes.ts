...
export type GuideScene = "activate-composer" | "sample-dimensions" | "create-project" | "source-scope" | "open-explore" | "expand-source" | "workspace" | "import" | "folder" | "explore" | "annotate" | "edit" | "analyse" | "dimensions" | "collections" | "profile" | "profile-select" | "batch" | "script" | "storyboard" | "video";
...
export const ONBOARDING_ROUTES: Record<OnboardingRole, GuideStep[]> = {
  designer: [
    ...
    { scene: "source-scope", title: "认识素材的归属", body: "目前我们在项目中进行导入，则素材默认归为所打开的项目。你也可以在主界面导入素材/文件夹，那所导入的素材则属于全局素材。全局素材包含项目素材。", target: "[data-tour=source-scope]" },
...
export function parseRoleGuide(value: unknown): RoleGuideProgress {
  ...
  // Append the new lessons after a previously completed six-step designer route.
  if (Array.isArray(p.completedRoles) && p.completedRoles.includes("designer") && fresh.sessions.designer?.step === 5) {
    fresh.sessions.designer = { ...fresh.sessions.designer, step: 6, ready: false, stepStartedAt: Date.now(), tasks: {} };
  }
...
export function advanceRoleGuide(guide: RoleGuideProgress, baselineText: string, now = Date.now(), skip = false): RoleGuideProgress {
  ...
  if (!role || !original || (!original.ready && !skip) || guide.status !== "active") return guide;
  ...
  if (session.step < ONBOARDING_ROUTES[role].length - 1) return { ...guide,
    sessions: { ...guide.sessions, [role]: { ...session, step: session.step + 1, ready: false, baselineText, stepStartedAt: now, tasks: {} } } };
...
export function practiceReady(scene: GuideScene, session: RoleSession, evidence: PracticeEvidence): boolean {
  ...
  switch (scene) {
    case "workspace": case "source-scope": return true;
