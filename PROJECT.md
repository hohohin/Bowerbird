...
截至 2026-09-21，Mac 更新通道与官网入口已发布 **26.9.2002**；Windows 保持 26.9.1803（没有更新的现成安装包）。Worker、数据库及模型服务仍沿用下述既有部署记录，本次未变更。
...
> **工作流卡片阶段存档与编排助手开工（2026-09-23，源码未发布）：** 存档现有内容卡独立存图、引用生命周期恢复、生成历史、并行调度、错误定位、复制及本机 Agent DS 文本改写链路。工作流逻辑 20 项、隔离运行/调度/引用生命周期回归、桌面 TypeScript、Rust 工作流 11 项，以及 Worker 文本改写与统一处理器 16 项通过；未调用真实模型、未发布安装包或云服务。下一步按用户确认新增工作流专属 Agent 卡片，先用已有卡片编排，不扩展新的业务算子。

> **Agent DS 改为投递本机 DSH 队列并自动送达活会话（2026-09-22，本机 dev 实现+单测+传输实测）：** 按用户决策，创作板 Agent DS 不再自行拉起 harness，而是把「本条消息 + 参考图」写成 payload 落 `.agent-z/ds-inbox/pending/<毫秒>-<ulid>.json`，**并自动把同一内容作为一轮 user 消息投进用户正开着的那条 DSH 会话**（`POST http://127.0.0.1:3080/api/session.prompt`，`mode:"queue"`，可用 `BOWERBIRD_DSH_WEB_URL` 覆盖基址）——目标从「Bowerbird 拥有一个对话助手」改为**借用本机 harness 的全量工具面与既有上下文**，Bowerbird 端只保留「从素材库方便传图」与「看清实际投递原文」两件事。两条腿以**文件为权威**：先落盘（绝不丢件），再 best-effort 送达（8 秒超时，失败降级为「留在队列等人工 drain」并把原因回给前端）；送达结果回写 payload 的 `delivery` 字段。目标会话由 `session.list` 发现——**排除 `origin:"subagent"` 与 `blank:true`**、`cwd` 命中本仓库（两侧路径归一化后比较）、取 `updatedAt` 最大者。payload v1 = `{version,kind,createdAt,visualProfileId,message,text,images[{path,name}],delivery?}`：`message` 是创作板原文、`text` 是注入视觉规范 capsule 后的实际投递正文（无注入时相同）、`images` 是绝对路径 + 素材名（正文 `@素材名` 与 ULID 存储文件名的对号依据）；消费方处理完搬入同目录 `archive/` 留档，`pending/` 文件名排序即投递顺序。链路因此去掉 node 子进程、`apps/cloud/.env`、DeepSeek key 与 DSH profile 预检，命令返回 `{path,sessionId,sessionTitle,autoDelivered,notice}`、前端按是否送达给不同提示并立即解除 busy（不再有 `ds_status` 往返）；参考图以绝对路径 + 素材名投递，但**看图能力受会话模型限制**——本机 DSH 当前模型 `deepseek-v4-flash` 声明为纯文本，`read_image` 会被拒，像素级理解需改走能看图的模型或 Bowerbird `understand_asset`（`.agent-z/rpc`，方舟 Vision；已实测：桌面端取走请求后约 100 秒返回，结果以 `agentz-<时间>` 维度落进该素材）。旧 ACP 拉起路径（`agent-ds-chat-cli.ts`/`agent-ds-dsh.ts`/`profileMode "agent-ds"`）保留未接线待回收，`ds-session.json` 不再更新。Rust **7 项**单测（payload/投递正文/落盘与回写/会话选择/路径归一化），`cargo test --lib` **406 passed / 9 ignored / 0 failed**、桌面 TypeScript（`pnpm lint` = tsc）exit 0；**传输层真机实测**：按同一逻辑直接跑 `session.list`→`session.prompt`，3 个候选顶层会话（子代理正确排除）、选中当前会话、返回 `{"accepted":true}`。未真实运行新编译的桌面应用、未调用 provider、安装包未动。仍 dev-only/Windows debug，云端生产边界不变。

> **Agent DS 本机 dev 改为拉起本地 DSH harness（2026-09-21，本机实现+真实链路验证）：** 创作板 Agent DS 从手写 DeepSeek 直连循环改为与 Agent Z/G 同类的「拉起 agent harness」定位：每轮 detached CLI 经内存计量代理（`EvalMeteredProxyControl`，phase `agent_ds_local`、`allowImageContent` 仅本地放开）拉起钉版 DSH（`profileMode "agent-ds"`，可选模板组不进候选镜像与正典文件集），工具面仅 `dreamina_generate`/`understand_asset`（loopback 桥 → `.agent-z/rpc` 契约回桌面端执行）。deepseek-flash 按其多模态能力在本地 patch 声明 `inputModalities: [text, image]`，参考图直接附给模型（Files API 经代理 404 自动回退 inline data URL，≤6 张/6 MiB 内联上限，超出退化路径注记），`understand_asset` 保留为聚焦反推与入库维度；会话升级 `ds-session.json` v2（每轮 fresh DSH session + transcript 注入续聊，成功才写回）。旧直连循环及 7 个测试删除。agent-worker **361/361** + typecheck、spike profile **27/27**（新增真实 DSH agent-ds e2e）、桌面 Rust **391/391**；真实链路冒烟（真图+真 DeepSeek）：2 模型回合、3,243 input / 1,048 output tokens，直看图与工具反推对比符合预期。仍 dev-only/Windows debug，云端生产「DeepSeek 文本 Agent + 方舟 Vision 工具」边界与候选镜像不变。
...
**工作流编排助手范围（2026-09-23，开始实现）：** 新增独立“工作流助手”卡片，根据需求组合现有指令、文本 Agent、生成、技能、视觉规范及触发器；使用现有 Agent 通道，不新增领域 Runner。第一版优先开发版本机 DSH，模型只提交受校验的结构化编排，桌面负责实例化真实可编辑卡片与连线；搭建与执行分开，默认不自动提交生图/反推等模型任务。现有文本 Agent 保留文本处理职责；不扩大为整个画板助手。
