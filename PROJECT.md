...
### dreamina OAuth app spawn 不写 token（2026-07-24）
...
- 根因：**不明**。试过多种（headless+checklogin device_code 报「过期」/ 非 headless 不写 token / CREATE_NEW_CONSOLE 给 console / stderr null 防 pipe deadlock）均不通。dreamina CLI 的 OAuth（device flow + token exchange + 写 token）在 app 子进程环境下不完成，可能涉及 OAuth session 存储（~/.dreamina_cli/ 他处，非 credential.json）、Windows Credential Manager / DPAPI 绑定 console session、或 dreamina 内部 console 检查——多轮调试未定位。**关键**：`dreamina user_credit` 检测登录态在 app spawn（CREATE_NO_WINDOW）下正常（读 token work），仅 login 写 token 这步不通。
- 解决/绕过：app 内 OAuth 搁置。[DreaminaLoginDialog](apps/desktop/src/components/DreaminaLoginDialog.tsx) 引导用户终端 `dreamina login`（可靠）+ 浏览器授权 + 回 app「重新检测」（user_credit 检测正常）。调试残留 `dreamina_login`/`dreamina_check_login` 命令保留（dead，未删）。
- 教训：外部 CLI 的 OAuth/登录态在 GUI app 子进程 spawn 下可能有不可预期的环境依赖（console / session / credential store），命令行验证通≠app spawn 通。登录类操作引导用户终端跑更稳。
- 相关文件：[commands/jimeng.rs](apps/desktop/src-tauri/src/commands/jimeng.rs)（dreamina_login/check_login）、[DreaminaLoginDialog.tsx](apps/desktop/src/components/DreaminaLoginDialog.tsx)。
