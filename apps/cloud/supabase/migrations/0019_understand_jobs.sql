-- Durable Bowerbird Cloud understand (caption/autoname/classify) queue.
-- Mirrors 0015 generation_jobs: instructions and images stay only in private
-- short-TTL generation-temp objects; this table holds control-plane metadata,
-- hashes and the few-KB result text (nulled once content expires).

create table public.understand_jobs (
  id uuid primary key default extensions.gen_random_uuid(),
  user_id uuid not null references public.billing_accounts(id) on delete restrict,
  idempotency_key text not null check (char_length(idempotency_key) between 1 and 200),
  operation text not null check (operation in ('caption', 'autoname', 'classify')),
  ...
  -- 免费档「每日 10 次」公平使用计数是否已计入本任务；失败/取消时由结算 RPC 回补。
...
create index understand_jobs_user_status_idx
  on public.understand_jobs (user_id, status, created_at desc);
create index understand_jobs_claim_idx
  on public.understand_jobs (status, lease_expires_at, queued_at, created_at)
  where status in ('queued', 'leased', 'running');
create unique index understand_jobs_active_lease_idx
  on public.understand_jobs (lease_id) where lease_id is not null;
create index understand_jobs_content_expiry_idx
  on public.understand_jobs (content_expires_at) where deleted_at is null;

alter table public.understand_jobs enable row level security;
alter table public.understand_jobs force row level security;
revoke all on table public.understand_jobs from anon, authenticated;
grant select on table public.understand_jobs to authenticated;

create policy understand_jobs_select_own
on public.understand_jobs for select to authenticated
using (
  exists (
    select 1 from public.billing_accounts as account
    where account.id = understand_jobs.user_id
      and account.auth_user_id = (select auth.uid())
  )
);

create or replace function public.claim_understand_job(
  p_worker_id text,
  p_lease_seconds integer default 90
)
returns public.understand_jobs
language plpgsql
security definer
set search_path = ''
as $$
declare
  candidate_id uuid;
  claimed public.understand_jobs;
begin
  ...
  from public.understand_jobs as job
  ...
  update public.understand_jobs as job
  set status = 'leased',
...
create or replace function public.heartbeat_understand_job(
  p_job_id uuid,
  p_lease_id uuid,
  p_lease_seconds integer default 90
)
...
begin
  ...
  for update;
  if not found then
    raise exception 'unknown understand job' using errcode = 'P0002';
  end if;
  if locked.lease_id is distinct from p_lease_id
      or locked.lease_expires_at is null
      or locked.lease_expires_at <= now() then
    raise exception 'invalid or expired understand lease' using errcode = '55000';
  end if;
  if locked.status not in ('leased', 'running', 'cancel_requested') then
    raise exception 'understand status does not hold a lease: %', locked.status using errcode = '55000';
  end if;
...
create or replace function public.mark_understand_job_submitted(
  p_job_id uuid,
  p_lease_id uuid
)
returns public.understand_jobs
...
begin
  ...
  if locked.lease_id is distinct from p_lease_id
    ...
    raise exception 'invalid understand lease or status' using errcode = '55000';
  end if;
...
create or replace function public.complete_understand_job(
  p_job_id uuid,
  p_lease_id uuid,
  p_status text,
...
begin
  if p_status not in ('succeeded', 'failed', 'cancelled', 'outcome_unknown') then
    raise exception 'invalid terminal understand status' using errcode = '22023';
  end if;
  if p_status = 'succeeded' and (
      p_result_text is null or btrim(p_result_text) = '' or char_length(p_result_text) > 65536
  ) then
    raise exception 'successful understand requires result text' using errcode = '22023';
  end if;
  ...
  if locked.status in ('succeeded', 'failed', 'cancelled', 'outcome_unknown') then
    if locked.status <> p_status then
      raise exception 'understand job already completed as %', locked.status using errcode = '55000';
    end if;
    return locked;
  end if;
  if locked.lease_id is distinct from p_lease_id
  ...
  if p_status = 'succeeded' then
    ...
    if locked.quota_consumed then
      perform public.undo_understand_quota(locked.user_id);
    end if;
  else
...
create or replace function public.reconcile_stale_understand_jobs(p_limit integer default 100)
returns integer
language plpgsql
security definer
set search_path = ''
...
revoke execute on function public.claim_understand_job(text, integer) from public, anon, authenticated;
revoke execute on function public.heartbeat_understand_job(uuid, uuid, integer) from public, anon, authenticated;
revoke execute on function public.mark_understand_job_submitted(uuid, uuid) from public, anon, authenticated;
revoke execute on function public.complete_understand_job(uuid, uuid, text, text, text, text) from public, anon, authenticated;
revoke execute on function public.reconcile_stale_understand_jobs(integer) from public, anon, authenticated;

grant execute on function public.claim_understand_job(text, integer) to service_role;
grant execute on function public.heartbeat_understand_job(uuid, uuid, integer) to service_role;
grant execute on function public.mark_understand_job_submitted(uuid, uuid) to service_role;
grant execute on function public.complete_understand_job(uuid, uuid, text, text, text, text) to service_role;
grant execute on function public.reconcile_stale_understand_jobs(integer) to service_role;

comment on table public.understand_jobs is
  'Durable Cloud understand jobs. Instruction/image content lives only in private generation-temp objects; result text is nulled after content expiry.';
comment on column public.understand_jobs.upstream_submitted_at is
