...
- [dev-doc/AGENT-RUNTIME-PLAN.md](dev-doc/AGENT-RUNTIME-PLAN.md) — Bowerbird 受限 VPS Worker 与内置 Skill Agent Runtime 专项计划（首版 `bowerbird-controlled-image-edit`：纯文本意图分析 → Skill 动态规划 → 审批 → Cloud 或本机 CLI 工具执行 → 反馈诊断与修订；Agent Kernel / Supabase / 方舟 / 桌面会话 UI，v1.10 2026-08-21，见 [apps/agent-worker](apps/agent-worker/)）
...
**当前阶段（2026-08-21 快照）：**
**最新 Agent 联调修正：** 三参考图普通表达已通过底图角色与输出角色回归；本轮进一步修复主体比例、接受后入库和黑箱式执行展示。“自动”比例现在由纯文本意图分析确定 `finalSubjectReferenceId` 后读取该主体引用的尺寸比例，显式选择优先，方舟按对应明确宽高生成；Agent 时间线展示意图判断摘要、计划、步骤开始/完成和每张过程图；云端已接受状态可恢复，全部用户可见控制图、阶段图和最终图经校验后以同一 `conversation_id` 组图入本地素材库。`agent-run` 与 VPS 三循环已重部署，待最后一轮桌面人工验收。
...
**测试基线：** 桌面 `cargo test` 现为 171 通过 + 2 ignored（Agent Z 真机 e2e，需交互桌面会话），前端 `tsc --noEmit` 与 production build 通过；agent-worker **112/112** + `tsc --noEmit`；Rust `cargo fmt --check` 与 `git diff --check` 通过；`bowerbird-controlled-image-edit` `0.1.1` 指令 bundle hash/loader 校验。Agent `agent-run` / `agent-worker` Edge Functions 的既有 Deno type-check、真实 Supabase/VPS/DeepSeek/方舟 Vision/Seedream 双审批修订 E2E、专业/普通表达两种真实计划拒绝 0 积分结算 E2E、终态积分结算（11 分）与产物 SHA-256 校验基线仍有效；本轮 Codex/即梦本地任务协议因当前机器没有 Deno 命令未复跑 Edge type-check，也尚未部署 `0028`–`0030` 或执行真实 CLI E2E（2026-08-21）。Skill quick validator 因当前 Python 环境缺少 PyYAML 未执行。扩展 Node tests 10/10、官网 build 基线沿用 2026-08-13 存档。
