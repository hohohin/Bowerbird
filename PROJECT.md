...
**自动更新：** Windows x64 已发布 **26.9.1803**，含启动时检查更新、设计师路线开放范围及应用内账号升级入口；M 系列 Mac 保持 **26.9.1802**，尚未包含这些新增客户端功能。官网检查、完整包下载和验签已通过，Intel、Mac 公证及双端原生升级仍待验收。发布/回滚和双端交接见 [桌面自动更新](dev-doc/DESKTOP-UPDATES.md)。

- **桌面**：当前 Windows 已发布 **26.9.1803**，M 系列 Mac 为 **26.9.1802**（SQLite 至 `0030`）。在此前 26.9.17 的项目画板、多线程、身份引导、素材仅画板可见、分层编辑、Mac 共用更新、本地分类代理/GPU 与标签恢复基础上，合并工作区既有改动及设置内自动更新；采用用户确认的整包发布范围。老版本仍需先手动安装一次 26.9.18，之后由设置检查后续新版。
...
> **26.9.1802 双端更新正式发布（2026-09-18）：** 发布 Windows x64 与 M 系列 Mac 的引导文案和第 7.1 步更新；两端实际包版本、哈希、公钥连续性、签名和篡改拒绝通过。官网清单及 Windows 下载入口切换到 26.9.1802，Mac 沿用 R2 直下，保留旧包与 26.9.18 回滚目录。候选、正式服务及公网发布链路通过；Intel、Mac 公证和真实安装重启仍待验收。完整发布/回滚与证据统一见 [桌面自动更新](dev-doc/DESKTOP-UPDATES.md)。

> **Windows 26.9.1802 本地签名包（2026-09-18）：** 按用户确认以 26.9.1802 表示 9 月 18 日第 2 版，纳入设计师引导文案及 7.1、暂停/重启恢复。修复 Windows/prepare.ps1 按目录名排除 Windows 导致小写 windows 平台源码漏拷的问题，改为根目录绝对路径。安装包 83177187 bytes，SHA-256 97d75f3007115cf71b8dc4a2a674e5e47531ebb55f7a43fff23ccebe25383c79；签名、清单版本、篡改拒绝、528 个构建输入、43 个引导资源及解包 x64/产品版本校验通过。Rust 合计 361 passed / 8 ignored / 11 filtered（私有 CLI 协议测试因沙箱权限限制单独非沙箱通过），隔离 NSIS 安装/重装/失败重试钩子、引导 14/14、引导与更新 UI 及 TypeScript/Vite 通过。保留独立凭据与安装清登录机制；旧 26.9.18 包保留。证据在 .tmp/package-20260918-2/；仅本地交付，未安装到用户环境或发布官网，线上 Windows/Mac 仍为 26.9.18。
