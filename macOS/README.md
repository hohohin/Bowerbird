...
**发布进度（2026-09-20，应急走 R2；COS+CDN 迁移推迟）：** 原计划自本版起迁移腾讯 COS + CDN（`cdn.bowerbird.cn`），但控制台前置（建桶/CDN 域名/证书/CNAME）尚未完成、`cdn.bowerbird.cn` 不可达，按用户决策 26.9.2002 **先走 R2 应急首发**：两份清单包 URL 回指 r2.dev 直下，上传/换清单流程沿用 R2。步骤：①创建 `macOS/.signing/r2.env`（R2_ACCOUNT_ID / R2_ACCESS_KEY_ID / R2_SECRET_ACCESS_KEY / R2_BUCKET 四行）后在仓库根执行 `/tmp/bowerbird-release-26.9.2002/upload-r2.sh`（上传双架构各六件、本地哈希断言、公网完整下载哈希 + minisign 复核、确认 26.9.18/1802/1901 历史包仍在）；②服务器 106.55.44.143 依次执行 `swap-manifest-aarch64.sh`（备份 26.9.1901 清单 → 哈希断言 `2c6b6158…` → rename 原子替换 → 公网复核）与 `swap-manifest-x86_64.sh`（Intel 通道首次上线：核验 `16f35772…` 后新建清单，回滚即删文件回到 404）。首页 Mac DMG 按钮指向 R2 的 26.9.2002 DMG（仓库两份首页已改，随候选→原子切换发布）。COS 就绪后的迁移不变（见 [DESKTOP-UPDATES.md](../dev-doc/DESKTOP-UPDATES.md)）：届时用 cdn URL 重新生成双架构清单、镜像历史包再各自原子切换一次。本轮未运行真实应用内更新。
