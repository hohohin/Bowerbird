-- 0052 — Unified Agent HTML 结果的原子结算主产物守卫（UNIFIED-AGENT-HARNESS U6）
-- 与 agent-worker Edge 的等待反馈规则保持一致：unified Agent 优先使用唯一
-- final_result；没有 final_result 时，允许唯一 viewport/full-page 主截图。
...
begin
  ...
  if p_final_status = 'succeeded' then
    ...
    if locked.skill_id = 'bowerbird-html-layout-render' then
      ...
      if final_count <> 1 and not (final_count = 0 and html_primary_count = 1) then
        raise exception 'succeeded agent run requires exactly one primary result'
          using errcode = '55000';
      end if;
...
comment on function public.settle_agent_run(uuid, uuid, text, text, text) is
  'Atomically settles Agent usage and its hold; success requires the Skill-specific primary result role, including unified HTML fallback.';
