...
create trigger if not exists on_auth_user_created_initialize_bowerbird
...
create policy if not exists subscriptions_select_own on public.subscriptions for select to authenticated using ((select auth.uid()) = user_id);
create policy if not exists orders_select_own on public.orders for select to authenticated using ((select auth.uid()) = user_id);
create policy if not exists credit_lots_select_own on public.credit_lots for select to authenticated using ((select auth.uid()) = user_id);
create policy if not exists credit_holds_select_own on public.credit_holds for select to authenticated using ((select auth.uid()) = user_id);
create policy if not exists credit_hold_allocations_select_own on public.credit_hold_allocations for select to authenticated using (exists (select 1 from public.credit_holds h where h.id = credit_hold_allocations.hold_id and h.user_id = (select auth.uid())));
create policy if not exists credit_transactions_select_own on public.credit_transactions for select to authenticated using ((select auth.uid()) = user_id);
create policy if not exists user_credits_select_own on public.user_credits for select to authenticated using ((select auth.uid()) = user_id);
create policy if not exists service_costs_select_active on public.service_costs for select to anon, authenticated using (active);
