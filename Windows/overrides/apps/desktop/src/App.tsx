...
function App() {
  ...
  // 浏览器扩展会在加载后及之后每 15 秒向本地 WS 发 ping；收到后标为已连接。
  useEffect(() => {
    let unlisten: UnlistenFn | undefined;
    listen("collect://extension-connected", () => setExtensionConnected(true)).then((u) => (unlisten = u));
    return () => unlisten?.();
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <main className="flex flex-1 flex-col overflow-hidden bg-canvas">
          ...
          <div className="relative flex-1 overflow-hidden">
            {showDetail ? <AssetDetail /> : total === 0 ? <WelcomePanel /> : <MasonryGrid />}
