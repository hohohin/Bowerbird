-- 0046 — HTML 离线排版 Run 原子结算主产物守卫（HTML-RENDER-PLAN H0 生产验收）
-- 图片 Skill 仍要求恰有一个 final_result；HTML Skill 要求 viewport/full-page
-- 主截图合计恰有一个。usage、hold 与 terminal status 仍由同一事务结算。
...
begin
  ...
  if p_final_status = 'succeeded' then
    ...
    where artifact.run_id = p_run_id
      ...
      and (
        (locked.skill_id = 'bowerbird-html-layout-render'
          and artifact.role in ('viewport_screenshot', 'full_page_screenshot'))
        or
        (locked.skill_id <> 'bowerbird-html-layout-render'
...
comment on function public.settle_agent_run(uuid, uuid, text, text, text) is
  'Atomically settles Agent usage and its hold; success requires the Skill-specific primary result role.';
