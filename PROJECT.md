...
- [dev-doc/AGENT-RUNTIME-PLAN.md](dev-doc/AGENT-RUNTIME-PLAN.md) — Bowerbird 受限 VPS Worker 与内置 Skill Agent Runtime 专项计划（首版 `bowerbird-controlled-image-edit`：纯文本意图分析 → Skill 动态规划 → 审批 → 工具执行 → 反馈诊断与修订；Agent Kernel / Supabase / 方舟 / 桌面会话 UI，v1.8 2026-08-21，见 [apps/agent-worker](apps/agent-worker/)）
...
**测试基线：** 桌面 `cargo test` 现为 168 通过 + 2 ignored（Agent Z 真机 e2e，需交互桌面会话），前端 `tsc --noEmit` 与 production build 通过；agent-worker **106/106** + `tsc --noEmit`；`bowerbird-controlled-image-edit` `0.1.1` 指令 bundle hash/loader 校验；Agent `agent-run` / `agent-worker` Edge Functions Deno type-check、真实 Supabase/VPS/DeepSeek/方舟 Vision/Seedream 双审批修订 E2E、专业/普通表达两种真实计划拒绝 0 积分结算 E2E、终态积分结算（11 分）、产物 SHA-256 校验与 `git diff --check` 均通过（2026-08-21）。Skill quick validator 因当前 Python 环境缺少 PyYAML 未执行。扩展 Node tests 10/10、官网 build 基线沿用 2026-08-13 存档。
