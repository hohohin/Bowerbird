...
**自动更新：** Windows x64 已发布 **26.9.1803**，含启动时检查更新、设计师路线开放范围及应用内账号升级入口；M 系列 Mac 线上为 **26.9.1901**（2026-09-20 确认清单已替换上线，含上述新增客户端功能与本地分类防误标），**26.9.2002 双架构（aarch64 + Intel 首发）已上传 R2 并完成公网核验，待服务器换双通道清单发布**——向量匹配判官、Jev 云端校验、画板/文本卡增强、钥匙串弹窗修复；COS+CDN 前置未就绪，本版按用户决策先走 R2 应急首发，COS 就绪后再迁移切换。官网检查、完整包下载和验签已通过，Intel、Mac 公证及双端原生升级仍待验收。发布/回滚和双端交接见 [桌面自动更新](dev-doc/DESKTOP-UPDATES.md)。
...
> **Mac 26.9.2002 双架构更新包（2026-09-20，本机构建待上传；Intel 通道首发）：** 26.9.2001 在本机核验后、上传前被同日第 2 包取代——工作区随后新增钥匙串弹窗根治（refresh token 内存缓存 + 自签名稳定身份 `Bowerbird Local Code Signing` + `hardenedRuntime: false`）并把 Mac 分发迁移至腾讯 COS+CDN（`cdn.bowerbird.cn`，R2 r2.dev 仅限开发且有限速；R2 桶与历史 URL 原样保留），发布包为纳入这些修复并统一双架构版本，按同日序号规范升为 **26.9.2002**（> 线上 26.9.1901，护栏通过；2001 从未上传，本地产物已删除）。aarch64 走 `release.sh`（自动检测稳定签名身份，首个非 ad-hoc 发布包）；Intel 在 Apple Silicon 本机 `rustup target add x86_64-apple-darwin` 后 `pnpm tauri build --target x86_64-apple-darwin` 交叉编译，并修复 vector.rs 平台常量在未支持目标缺失的编译错误（占位常量 + install 护栏保证不可达）。构建前 Rust 全量 **384 passed / 6 ignored**、画板纯逻辑 127/127、local-classification UI 回归与 TypeScript 通过。产物（`macOS/dist/`）：aarch64 更新包 **86,394,528 bytes**，SHA-256 `03ef60428b9118efe7812ea8d3c57dcb255723ae63422c7e9f70dcc8c7b32ee3`，DMG `5fd72b14205ba8600cd0f8f43470d70377c360de96ec7b8ab07ab1ae5b4354df`；x86_64 更新包 **86,851,474 bytes**，`80f4c2f023e745f4315189f61407b5b9ef987e15143432e0aba46492fd592e9d`，DMG `725073a0ef04691155cafe78dcfd599aae1c9a33d6347f6b25fcfa2e5ca5b621`，清单 `16f35772060bb0c124bac84ce74838035a2f665f612e293cab65cd34dfeea7b7`；aarch64 清单 `2c6b61588a1d7254ade6a4fe4e4f9a6f55ab090ae41fd4867daf87817adba13a`。清单 URL 因 COS+CDN 未就绪按用户决策回指 R2 r2.dev 直下（应急首发；COS 就绪后用 cdn URL 重新生成并切换，原 cdn 版清单哈希作废）。双架构核验通过：版本 26.9.2002、Mach-O 架构、内嵌 Mac 公钥与已发布连续（key id `0b5a2efc4865664f`）、`codesign` Authority 为稳定身份且 `--verify --deep` 通过、DMG `hdiutil verify`、minisign 主/全局签名、篡改拒绝、清单与 `.sig` 逐字节一致；Intel 向量包不可用（onnxruntime 无 x86_64 资产）回退 VLM，未在真实 Intel 机验收。**已上传 R2 并完成公网核验（2026-09-21：本地哈希断言、公网完整下载哈希、minisign、双清单哈希、HEAD 大小、历史包仍在；当晚 r2.dev 速度波动，慢路由可能触发客户端 300 秒下载超时需重试），待服务器替换/新建双通道清单**（交接件 `/tmp/bowerbird-release-26.9.2002/`：swap-manifest-aarch64.sh + swap-manifest-x86_64.sh）。细节见 `macOS/README.md` 与 DESKTOP-UPDATES.md「Mac 26.9.2002 双架构发布侧核验」。

> **视频创作链路去除本机 ffmpeg 硬依赖（2026-09-20，已纳入 Mac 26.9.2002）：** 修复“创作视频要求本机装有 ffmpeg/ffprobe”的体验问题：提交预检与孤儿任务取回不再做工具硬门禁（`ensure_video_tools` 已删除），mp4/mov/m4v 的宽高/时长改由纯 Rust `mp4` crate 进程内解析（`media/probe.rs`：mvhd 取时长、视频轨取宽高，解析失败或 webm/mkv/avi 罕见格式回退本机 ffprobe），参考视频 2–30 秒校验与生成/导入入库在干净机器全链路可用；缩略图失败时导入路径与生成路径统一落 `thumb_path: None`，库网格（MasonryGrid）对无缩略图视频渲染 `<video preload="metadata">` 首帧（与画板一致），非视频缺缩略图维持格式占位，已装 ffmpeg 的机器行为不变（缩略图照常抽取）。新增依赖 `mp4 0.14.0`（纯 Rust，传递仅 num-rational/num-bigint；Windows 离线构建需同步 Cargo.lock）。验证：Rust 全量 **384 passed / 0 failed / 6 ignored**（新增原生解析两测与 `generated_video_ingests_without_local_ffmpeg` 干净机器入库回归——显式失效 `BOWERBIRD_*_BINARY` 禁止回退搜索，夹具由 mp4 writer 离线生成）、TypeScript、explorer/layer-editor/layer-viewport/annotation-tools 与 explorer 逻辑回归通过；既有 ignored PATH 测试改为仅要求 ffmpeg。集合面板/生成参考条等对缺缩略图素材回退 store_path 的 `<img>` 行为未动（视频可能破图，预存边缘）。

> **Mac 26.9.2001 更新包（2026-09-20，上传前被 26.9.2002 取代）：** 收录 9 月 19/20 存档工作——本地分类向量精确匹配判官（jina-clip-v2 int8 可选安装）与可选云端最终校验 Jev、画板新卡锚定可视区左上角、视频卡独立样式、文本卡表格增强（标题编辑/整表 CSV 复制/行列尺寸拖拽）、创作模式框选、mac「打开所在文件夹」选中与方向键乱码修复；按同日序号规范定为 **26.9.2001**（> 线上 26.9.1901——1901 清单已由发布侧替换上线，2026-09-20 确认，护栏通过）。构建前 Rust 全量 **379 passed / 6 ignored**、画板纯逻辑 **127/127**、creation-editor/creation-marquee/notes/media/reference/local-classification 六组 UI 回归与 TypeScript 通过。`release.sh`（R2_SKIP_UPLOAD=1，本机缺 r2.env 凭据）完成签名构建，产物在 `macOS/dist/`：更新包 `Bowerbird_26.9.2001_aarch64.app.tar.gz` **86,344,811 bytes**，SHA-256 `de431a5fdc9da85cda13df2901df3ab54b1e075c2b793bb5fe077179d19931e0`；手动 DMG **86,881,253 bytes**，`852fe0edd1d2304c4761a583bf3f75f43f2f049c1b537acf6e0f139f7220cfa5`；清单 `darwin-aarch64.json`（R2 直下，SHA-256 `ed68d36aecf2f6c5ae3d2c18c670a4e71ff238af2e06ebd9f9f80ef8b27de46a`）。本机核验：包内版本/架构/arm64、内嵌 Mac 公钥与已发布 1802/1901 逐字节一致（key id `0b5a2efc4865664f`）、DMG `hdiutil verify`、minisign 主签名与全局签名、篡改字节拒绝、清单 signature 与 `.sig` 逐字节一致全部通过。**待恢复 R2 凭据后上传并替换官网清单**（交接件 `/tmp/bowerbird-release-26.9.2001/`：upload-r2.sh → 服务器 swap-manifest.sh，备份 26.9.1901）；本轮未运行真实应用内更新。细节见 `macOS/README.md` 与 DESKTOP-UPDATES.md「Mac 26.9.2001 发布侧核验」。（2026-09-20 追记：该包在上传前被同日第 2 包 26.9.2002 取代——纳入钥匙串修复并迁移 COS 分发；2001 从未上传任何存储，本地产物与交接脚本已作废删除，后续见「Mac 26.9.2002 双架构更新包」。）
...
**2026-09-20 Mac 分发迁移腾讯 COS+CDN（26.9.2002 起生效）：** Mac 更新包与 DMG 发布到腾讯 COS + CDN 域名 `cdn.bowerbird.cn`（`mac_package/` 键名，与 R2 相同布局），历史包 26.9.18/1802/1901 同字节镜像；R2 桶与 r2.dev URL 原样保留（已发布历史清单仍指向，不可删除或关公开访问），不再向 R2 发新包（Cloudflare 声明 r2.dev 仅限开发且有限速）。构建机凭据在 `macOS/.signing/cos.env`（模板 `cos.env.example`），`release.sh` 一键上传；官网清单仍由发布侧核验后原子替换，上传 ≠ 发布。协议、前置准备与交接见 [DESKTOP-UPDATES.md](dev-doc/DESKTOP-UPDATES.md)。同日起 Mac 发布包以稳定本地签名身份 `Bowerbird Local Code Signing` 签名（根治钥匙串弹窗，非 Developer ID 公证），Intel（darwin-x86_64）通道自 26.9.2002 首次发布，可在 Apple Silicon 本机交叉编译。**应急例外（2026-09-20）：COS+CDN 前置未完成，26.9.2002 双架构按用户决策仍走 R2 应急首发（清单回指 r2.dev）；COS 就绪后重新生成清单、镜像历史包再切换，迁移方向不变。**
...
**2026-09-18 桌面自动更新：** 采用 Tauri 官方 updater、固定平台公钥和不可变签名包；官网统一检查入口，按平台发布版本清单。共享客户端源码已增加每次启动静默检查，新版弹窗由用户选择下载、安装或本次跳过；无更新或检查失败不弹窗，设置保留手动检查。该启动行为已纳入 Windows 26.9.1803；Mac 现役 26.9.1802 尚不含此能力。更新专项细节集中维护于 [DESKTOP-UPDATES.md](dev-doc/DESKTOP-UPDATES.md)，包括 Windows 清登录边界、Mac 独立签名及 R2 下载、发布回滚与双端共识；本页只保留项目摘要。
...
### x86_64-apple-darwin 交叉编译因 vector.rs 平台常量缺失失败（2026-09-20）

在 Apple Silicon 本机为 Intel 通道交叉编译（`pnpm tauri build --target x86_64-apple-darwin`）时，`local_classification/vector.rs` 报 8 个 E0425：`RUNTIME_ARCHIVE/RUNTIME_HASH/RUNTIME_BYTES/RUNTIME_MEMBER` 未定义。这四个 onnxruntime 常量只在与 `vector_supported()` 匹配的平台（macOS arm64 / Windows x64）用 `#[cfg]` 定义；Intel Mac 属「不支持向量包」的目标，常量不存在，但运行库下载函数无条件引用它们（`install()` 开头的运行时护栏不做编译期裁剪）。修复：为未支持目标补占位常量定义（`#[cfg(not(any(…)))]` 空值/0），既有 `vector_supported()` 护栏保证占位值永远不可达；`cargo check --target x86_64-apple-darwin` 通过后 Intel 包正常构建。教训：按平台 `cfg` 定义常量时，引用处要么同样门控，要么给所有目标兜底定义——运行时护栏挡不住编译期名字解析。
