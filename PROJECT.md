...
**测试：** `cargo test` 25 通过（导入/去重/多图过滤 + assemble_pack + FTS5 文件名搜索 + analyses 读写 + caption sections 解析 + list_prompted_assets + **采集即命名 `clean_name`/`split_name_and_desc` + caption 共享模块 + 生成图取图快照差分 `list_new_generated`**）；前端 `tsc --noEmit` 通过。
