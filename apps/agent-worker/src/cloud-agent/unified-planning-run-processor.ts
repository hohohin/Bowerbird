...
export function requiredDeliverySteps(goal: string): Array<"compose_html" | "render_html" | "inspect_artifact" | "finalize_output"> {
...
export function requiredDeliveryPolicyContext(goal: string): Record<string, unknown> | undefined {
  const approvedPlanStepKinds = requiredDeliverySteps(goal);
  if (!approvedPlanStepKinds.length) return undefined;
  return {
    schemaVersion: 1,
    mode: "exact_approved_plan_sequence",
    reason: "deterministic_exact_copy_detail_page",
    approvedPlanStepKinds,
    planningOnlyTools: ["list_run_assets", "understand_asset"],
    forbiddenApprovedPlanStepKinds: ["understand_asset", "generate_image", "compose_xiaohongshu"],
    instruction: "plan.steps must contain exactly the four approvedPlanStepKinds in that order, with no extra steps; planning-only tools never appear in plan.steps; do not generate a replacement image for this delivery.",
  };
}

export function requiredExactCopyLines(goal: string): string[] {
  if (!requiredDeliverySteps(goal).length) return [];
  const marker = "参考内容：";
...
export class UnifiedPlanningRunProcessor implements AgentRunProcessor {
  ...
  async process(context: AgentRunContext): Promise<void> {
    ...
    try {
      ...
      const deliveryPolicy = requiredDeliveryPolicyContext(checkpoint.input.goal);
      const bridge = createUnifiedPlanningToolBridge({
        ...
        requiredStepKinds: deliveryPolicy?.approvedPlanStepKinds as HarnessPlanStep["kind"][] | undefined,
      ...
      const result = await runner.run(checkpoint, [{
        ...
        text: [
          ...
          ...(deliveryPolicy ? [
            "[BOWERBIRD_REQUIRED_DELIVERY_POLICY_V1 trust=system]",
            canonicalJson(deliveryPolicy),
            "[/BOWERBIRD_REQUIRED_DELIVERY_POLICY_V1]",
          ] : []),
          "[BOWERBIRD_USER_GOAL_V1]",
          canonicalJson({ goal: checkpoint.input.goal, ratio: checkpoint.input.ratio, htmlOutput: checkpoint.input.htmlOutput }),
          "[/BOWERBIRD_USER_GOAL_V1]",
        ].join("\n"),
      }]);
  ...
  private async executeApprovedHtml(
    ...
    try {
      ...
      const composeDefinition = createApprovedComposeHtmlToolDefinition({
        ...
        requiredTextLines: requiredExactCopyLines(checkpoint.input.goal),
        control: context.control,
        workspace,
      });
      ...
      const result = await runner.run(checkpoint, [{
        ...
        text: [
          ...
          shape.xiaohongshu
            ? "Execute exactly the approved compose_html, render_html, optional inspect_artifact, compose_xiaohongshu, then finalize_output steps. Echo the raw artifact ids only in compose_html.resourceArtifactIds. Inside HTML, use only the supplied asset:reference-N aliases; never put a raw artifact id after asset:. render_html, inspect_artifact and finalize_output take {}."
            : "Execute exactly the approved compose_html, render_html, optional inspect_artifact, then finalize_output steps. Echo the raw artifact ids only in compose_html.resourceArtifactIds. Inside HTML, use only the supplied asset:reference-N aliases; never put a raw artifact id after asset:. Every tool except compose_html takes {}.",
          ...
          "For deterministic exact-copy delivery, every non-empty source line after 参考内容： and before ::: must remain a contiguous visible text sequence after whitespace normalization. Preserve every punctuation mark; do not split one source line into separately worded headings or subtitles. The parent validates this mechanically before any durable compose side effect.",
          ...(shape.xiaohongshu ? [XIAOHONGSHU_OUTPUT_RECIPE_V1] : []),
          canonicalJson({
            goal: checkpoint.input.goal,
            requiredExactCopyLines: requiredExactCopyLines(checkpoint.input.goal),
            ...
            approvedPlan: plan,
            resourceArtifactIds: shape.compose.inputAssetIds,
            htmlAssetReferences: shape.compose.inputAssetIds.map((resourceArtifactId, index) => ({
              resourceArtifactId,
              htmlSrc: `asset:reference-${index + 1}`,
            })),
          }),
          shape.xiaohongshu ? "[/BOWERBIRD_APPROVED_CONTENT_EXECUTION_V1]" : "[/BOWERBIRD_APPROVED_HTML_EXECUTION_V1]",
        ].join("\n"),
