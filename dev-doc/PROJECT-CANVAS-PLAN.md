...
17. 新开空项目时默认处于浏览模式：点击单素材打开原图，点击素材组打开组图轮播；聚焦创作框后，同样点击改为加入创作框，退出创作模式后立即恢复预览行为。浏览模式在空白处按下并向任意方向拖动可立即框选多个素材，Shift 拖动框选可追加，普通空白点击清空框选；全程保持系统默认箭头光标，创作模式下不得误触框选。进入创作模式后画板蓝框、顶部刘海和编辑框蓝边应同步出现，退出后同步消失。
