...
> 更新时间：2026-08-11 ・ 当前分支：`dev` ・ P6.1、P9-T5、0011 与支付延期记录已随本次存档提交。

---

## 一句话现状

收费化架构（账号 / 积分 / 托管算力 / 订阅支付 / 功能门控，对应计划 P0–P8）已完成代码、P9-T2 全量回归、P9-T3 安全审计与真实云端部署：Supabase `0001~0011`、5 个 Edge Functions、Cloud Secrets 均已上线；桌面与官网同账号登录/共享余额、东京 Seedream 出图、本地入库、来源标记、扣费流水、幂等认领、限流与成本熔断均已验收。P6.1 已把免费仅 Cloud、Pro/Studio 才可使用本机 CLI 的门控收口到 UI、store、Rust command、理解/自动分析及任务恢复层；P9-T5 已把官方 Cloud URL/publishable key 改为构建期内置，移除用户可编辑技术选项。真实支付因备案/商户资质前置周期由用户明确暂停，支付保持 Mock。
...
- Bowerbird Supabase 项目已链接并推送 `0001~0011`；`0008` 修复注册初始化与 RLS，`0009` 修复 `credit_hold` 的 PL/pgSQL record/alias 遮蔽，`0010` 落地原子用量守卫，`0011` 修复跨日余额快照陈旧与每日额度未自动补发。
...
**2026-08-11 P9-T5 Cloud 正式配置收口：**桌面构建脚本只从构建环境或本地 gitignored `apps/cloud/.env(.local)` 读取 Supabase URL/publishable key，并通过编译期变量内置进官方构建；运行时不再读取或信任用户 `settings.json` 中的 endpoint/key。设置 UI 已移除 URL、publishable key、“启用云端”和“Mock 算力”，仅只读显示官方服务是否已配置，并保留 `cloud_auto_understand` 隐私授权。前端登录、生成、重试和理解门控统一读取 Rust `AuthSnapshot.cloud_available`；旧 settings 的五个连接字段启动时自动清理。Mock/真实算力仍只由 Edge Secret `BOWERBIRD_CLOUD_MOCK` 决定。验证：Rust 101/101、桌面 TypeScript/Vite production build 通过；全仓 `cargo fmt --check` 仍受 PROJECT 已记录的既有格式漂移影响，本轮未扩散格式化。
...
### 计划 P0–P8 全量（代码 + 测试）
- **P0 云脚手架**：`apps/cloud/`（Supabase 迁移 + Edge Functions + README/DEPLOY）；桌面 `src-tauri/src/cloud/` 边界（公开配置 + 共享 client）。早期 `cloud_enabled`/Mock 用户开关已在 P9-T5 正式配置收口时移除。
...
### 本会话新增（真实凭据接入）
- **Supabase 新 key 体系迁移**：`publishable`/`secret` 取代旧 `anon`/`service_role`。`_shared/auth.ts` 的 `namedKey()` 支持 `SUPABASE_PUBLISHABLE_KEYS`/`SUPABASE_SECRET_KEYS` JSON、直连 `SUPABASE_PUBLISHABLE_KEY`/`SUPABASE_SECRET_KEY`、并回退旧名；桌面构建也兼容 `SUPABASE_ANON_KEY` 作为旧公开 key 输入，但不写入用户设置。
- **桌面官方云配置内置（P9-T5）**：`src-tauri/build.rs` 从构建环境或 gitignored `apps/cloud/.env(.local)` 读取 URL/publishable key，编译进 `CloudConfig::official()`；用户 `settings.json` 不能覆盖 endpoint，设置页仅显示只读可用状态。
...
5. **密钥分级**：桌面端只持构建期内置、不可由用户修改的 publishable key + URL，refresh token 进 OS keychain；secret/方舟/支付密钥只在 Edge Functions。
...
| DB 迁移 | `apps/cloud/supabase/migrations/0001~0011` |
...
- 方舟 429 稳定性：仅对 `RequestBurstTooFast` 增加少量带随机抖动的退避重试，并把反推失败从开发者 Console 正式显示到 UI；当前失败会正确回滚，但需要用户手动重试。
