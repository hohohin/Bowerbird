//! 生成 provider 抽象；当前支持 Codex CLI、Dreamina CLI 与 Bowerbird Cloud。
//! 理解引擎由 understand 模块单独选择；产品边界见 PROJECT.md 约定 1。
...
/// 生成 provider。理解引擎另由 UnderstandProvider 选择。
/// 命令层经 [`resolve_gen_provider`] 按 `provider` 参数取实现（PROJECT.md 约定 1）。
#[async_trait]
pub trait GenProvider: Send + Sync {
    ...
    /// 能力自述：理解类（chat / caption）仅 codex；generate 由各实现支持（PROJECT.md 约定 1）。
...
/// 按 `provider` 参数取实现（PROJECT.md 约定 1）。
///
/// - `None | "codex" | "default"` → [`CodexCliProvider`]（参数缺省时的默认实现）；
...
/// - 其他 → 报错。
///
/// 调用方显式传入已选择的 provider；`None` 保持 codex 兼容默认值。
