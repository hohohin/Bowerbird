...
> **Mac 26.9.1802 引导更新包（2026-09-18，本机构建待发布）：** mac 合入 dev `56699d3`（官网 HEAD 修复 + 设计师引导文案与 7.1），按同日 patch 序号规范升至 **26.9.1802**（> 已发布 26.9.18）并按 DESKTOP-UPDATES 流程完成签名构建：aarch64 `.app.tar.gz`+签名、DMG 与 `darwin-aarch64.json` 清单在本机产出，引导契约 14/14、三身份引导与 v0917 包界面回归通过。产物未上传、清单未发布；发布后 26.9.18 客户端应用内升级即为 darwin 通道首次真实更新验收。产物哈希与边界见 `macOS/README.md`。
