...
- [dev-doc/UNIFIED-AGENT-HARNESS-PLAN.md](dev-doc/UNIFIED-AGENT-HARNESS-PLAN.md) — 通用云端 Agent Harness 新专项（一个 Bowerbird Agent + 多种受控 Tool/输出配方；Bowerbird Tool Gateway 与控制面权威；HTML/未来渠道能力禁止另造 Agent Runner，U0–U6，v1.51 2026-09-01；U2–U5 完成，U6 无内容观察、升级/回滚、SBOM/license 与镜像扫描证据已完成；自然样本不足且基础层仍有未修复 HIGH/CRITICAL，保持 test-only）
...
**U3 候选镜像验证基线（2026-09-01；已被真实执行修正超越）：** 每素材唯一视觉观察槽修正后的候选 `bowerbird/agent-worker-unified-harness:candidate` 曾重建为 `sha256:b238d6ef77c8a53707b7986ebf55e33be9113f2c19df02f51f3def356fe11864`，Docker inspect size 165,741,401 bytes，运行用户 `node`。449 个依赖及 506 条供应链策略检查通过；在 768 MiB、只读根文件系统、无网络、`no-new-privileges`、`cap_drop: ALL` 下连续两轮完整正式入口探针全绿。首次 OOM 根因是探针仍提交 v1、被新 v2 Policy 拒绝后 fake model 无限重放；fixture 升为 v2 后无需提高内存。本轮真实 HTML 执行又修改了计量代理、processor prompt、Vision prompt 和 DSH bridge，因此该 digest 不再代表当前源码；已由 U6 最终候选 `sha256:3e9efd07…dfa0dd` 取代。历史镜像仅在本地，未推送或部署。

**近期里程碑（只保留最近 3 条；更早的全量历史见 [dev-doc/进展归档.md](dev-doc/进展归档.md)）**

> **通用 Agent Harness U6：观察入口与供应链/镜像证据完成（2026-09-01；进行中、保持 test-only）**：无内容 reporter 已按 runtime 聚合失败/取消、p50/p95、credits、`provider_cost_micros`、tool retry/`outcome_unknown`、call/usage identity 重复；质量和 crash/re-claim 只接受显式 paired evidence。U4 上线后 `2026-09-01T07:30:00Z` 起远端为 legacy 5 / DSH 2 个 terminal Run，call/usage 无重复、终态积分对账全 PASS；legacy 两个失败是故意晚杀 `provider_outcome_unknown`，DSH 为 1 成功/1 取消，不能当自然成功率。冻结的 18-case + 每侧 1 个真实 crash/re-claim 给出两侧策略 77.8%、结构化 100%、recovery 100%，但自然样本仍不足。Runbook、SPDX 2.3 与 license audit 已完成；最终安装树 448 个唯一 package、189 个 DSH package、缺失 license 声明 0，lock SHA-256 `6e714f14…fd19`。包含 U5 的最终候选 `sha256:3e9efd07…dfa0dd`（165,800,960 bytes，用户 `node`）在 read-only rootfs、`network none`、drop ALL、no-new-privileges、无 host port/无 Docker socket 下通过 planning/controlled processor 完整探针。Trivy `0.70.0@sha256:be1190…a41e` 以断网只读 tar 扫描；删除运行时 npm/corepack/pnpm 并将全部旧 `js-yaml` 收敛到 `4.3.1` 后，Node HIGH 7→0，secret 0；仍有当前官方 Debian 基础层 4 CRITICAL / 18 HIGH，均无 FixedVersion 且无人接受，因此不扩大开放。Worker **289/289** + TypeScript、Profile **21/21**、候选资产 **2/2**、release check 全通过。公开档位、预算/计费与默认 runtime 决策未授权；本轮未部署、未调用 provider、未产生费用。
...
**未开始 / 待办：**
- **通用 Agent Harness U6 — 进行中**：无内容观察、升级/回滚 Runbook、SPDX/license、包含 U5 的候选重建、隔离探针与断网漏洞/secret 扫描均已完成；可修复 Node HIGH 已清零。当前只剩两类门槛：继续积累 test-only 自然小名单/人工质量样本（现仅 DSH 2 条），以及等待官方基础镜像修复或由明确责任人接受剩余 4 CRITICAL / 18 HIGH。公开档位、预算/计费和 DSH 默认/双栈/停止三选一需另行产品决策，当前不扩大开放、不扩成小红书发布产品。
...
- 相关文件：[profile.test.mjs](spikes/unified-agent-harness-u1/test/profile.test.mjs)、[dsh-acp-port.mjs](spikes/unified-agent-harness-u1/profiles/bowerbird-u1/scripts/dsh-acp-port.mjs)、[runtime.mjs](spikes/unified-agent-harness-u1/profiles/bowerbird-u1/scripts/runtime.mjs)。
