...
- [dev-doc/UNIFIED-AGENT-HARNESS-PLAN.md](dev-doc/UNIFIED-AGENT-HARNESS-PLAN.md) — 通用云端 Agent Harness 新专项（一个 Bowerbird Agent + 多种受控 Tool/输出配方；Bowerbird Tool Gateway 与控制面权威；HTML/未来渠道能力禁止另造 Agent Runner，U0–U6，v1.52 2026-09-01；U2–U5 完成，U6 候选已 test-only 部署并通过零 provider 冒烟与首组真实 paired；自然样本不足且基础层仍有未修复 HIGH/CRITICAL，保持 test-only）
...
> **通用 Agent Harness U6：候选已 test-only 部署并通过首组真实 paired（2026-09-01；进行中、保持 test-only）**：无内容 reporter 已按 runtime 聚合失败/取消、p50/p95、credits、`provider_cost_micros`、tool retry/`outcome_unknown`、call/usage identity 重复；质量和 crash/re-claim 只接受显式 paired evidence。U4 上线后 `2026-09-01T07:30:00Z` 起远端基线为 legacy 5 / DSH 2 个 terminal Run，call/usage 无重复、终态积分对账全 PASS；legacy 两个失败是故意晚杀 `provider_outcome_unknown`，DSH 为 1 成功/1 取消，不能当自然成功率。冻结的 18-case + 每侧 1 个真实 crash/re-claim 给出两侧策略 77.8%、结构化 100%、recovery 100%，但自然样本仍不足。Runbook、SPDX 2.3 与 license audit 已完成；最终安装树 448 个唯一 package、189 个 DSH package、缺失 license 声明 0，lock SHA-256 `6e714f14…fd19`。本地候选 `sha256:3e9efd07…dfa0dd`（165,800,960 bytes）经同一白名单在 VPS 重建为 OCI index `sha256:58de8b87…3d3e8`（inspect 165,844,481 bytes，用户 `node`），断网只读 planning/controlled 双 processor 探针全绿；切换前后 queue/active/expired lease/TTL 均为 0，容器 restart 0、OOM false，安全/资源约束无漂移。零 provider smoke 完成普通账号拒绝、测试账号 DSH create/get/claim、runtime drift 409 与直接 cancel，明确 `provider_calls=0`。随后相同纯文本意图各跑 1 条真实 legacy/DSH：均 succeeded、1 个 final artifact、3 条成功 tool、7 credits、0 retry/`outcome_unknown`/重复 identity；legacy 93,684 ms / 268,180 provider-cost micros，DSH 95,049 ms / 277,214 micros。真实费用调用为两侧各一次 DeepSeek 规划链和一次 Seedream、0 Vision；最终健康恢复全绿。Trivy 修复后 Node HIGH 0、secret 0，但当前 Debian 基础层仍有 4 CRITICAL / 18 HIGH 且无 FixedVersion/接受人，因此不扩大开放。回滚镜像 `rollback-u6-20260901T1538Z` 与 0600 env 快照保留，含 env staging 和 `/tmp` 发布包已删除。公开档位、预算/计费与默认 runtime 仍待产品决策。
...
**未开始 / 待办：**
- **通用 Agent Harness U6 — 进行中**：无内容观察、升级/回滚 Runbook、SPDX/license、包含 U5 的候选重建/扫描、test-only 部署、零 provider smoke 与首组部署后真实 paired 均已完成；可修复 Node HIGH 已清零。当前只剩两类门槛：继续积累 test-only 自然小名单/人工质量样本（自然 DSH 仍仅 2 条；部署后 paired 是合成测试，不冒充自然流量），以及等待官方基础镜像修复或由明确责任人接受剩余 4 CRITICAL / 18 HIGH。公开档位、预算/计费和 DSH 默认/双栈/停止三选一需另行产品决策，当前不扩大开放、不扩成小红书发布产品。
