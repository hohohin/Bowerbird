...
截至 2026-09-24，工作流模板库、开发版编排助手及画板交互增量已完成源码存档复核。最近记录的发布版本仍为 Mac **26.9.2002**、Windows **26.9.1803**；本轮仅存档与推送，未构建安装包或变更云服务。Worker、数据库及模型服务沿用下述 9 月 22 日部署记录。

**自动更新：** Windows x64 已发布 **26.9.1803**，含启动时检查更新、设计师路线开放范围及应用内账号升级入口；M 系列 Mac 线上为 **26.9.2002**，发布说明包含钥匙串/视频依赖修复、向量匹配与 Jev 校验、画板/文本卡增强；本次发布已有 R2 包，不代表工作区最新未打包功能已上线。官网检查、完整包下载和验签已通过，Intel、Mac 公证及双端原生升级仍待验收。发布/回滚和双端交接见 [桌面自动更新](dev-doc/DESKTOP-UPDATES.md)。

- **桌面**：当前 Windows 已发布 **26.9.1803**，M 系列 Mac 为 **26.9.2002**（SQLite 至 `0030`）。在此前 26.9.17 的项目画板、多线程、身份引导、素材仅画板可见、分层编辑、Mac 共用更新、本地分类代理/GPU 与标签恢复基础上，合并工作区既有改动及设置内自动更新；采用用户确认的整包发布范围。老版本仍需先手动安装一次 26.9.18，之后由设置检查后续新版。
- **云端**：最近部署记录为 **9 月 22 日**：VPS Worker 以「现役镜像 + src 覆盖层」方式更新（视觉规范 schema 2 workflow 提炼、`workflow.md` 提示词、生成 transparent 透传、统一 Agent 新增公众号排版领域方法；镜像 `sha256:7f3813a80a52…`，回滚 `rollback-workflow-20260922`），Edge `visual-profile` v15（schema 2 + capabilities）、`generate-proxy` v37（transparent）。在 9 月 14 日的兑换码/后台（`0059/0060`）、分层（`0061`）与 DSH `deepseek-flash` 基线之上叠加，`0057` 仍为未集成预留；无新增云数据库迁移。既有观察：understand/visual 清理循环对控制面持续 500（先于本次部署存在，未定位）。版本与回滚入口见 [收费化进度](dev-doc/ARCH-ADJUST-PROGRESS.md)。
...
> **工作流模板、编排协议与画板交互统一存档（2026-09-24，源码未发布）：** 将本地模板库/参数化子流程、planning-v2 草稿校验与提交、引用及保存恢复、新画板初始化/拖动、生成产物连线与图片标注「添加到画板」一并存档。标注沿既有入库与坐标元数据链路创建独立图片卡，保存失败重试复用已入库素材。本次复核 9 组隔离浏览器回归全部通过，其中模板逻辑 34 项、编排协议 50 项、助手 48 项、引用 8 项；Rust 工作流筛选 22 项与 Agent DS 筛选 10 项均通过（含 1 项交集），桌面 TypeScript/Vite 构建与 diff 空白检查通过。源码包含 SQLite `0033` 迁移，但本轮未操作真实素材库、调用付费模型、打包或部署；原生桌面完整验收仍待完成。

> **本地工作流模板库与参数化子流程（2026-09-24，源码未发布）：** 支持画板框选后右键「存为模板」，确认是否带上素材/输入文案；操作指令始终保留。模板库支持命名/搜索/更新版本、公开输入与附加要求/画面比例参数，并在任意项目创建独立实例。子流程沿用现有卡片编译与执行器，统一入口配置和启动，内部步骤保持可编辑；已有实例固定模板快照，不随库更新或删除变化。按用户选择通过 `.bbworkflow.json` 导出/导入分享，接收者替换自己的素材与 provider；不携带原素材 ID、个人规范绑定或执行记录。SQLite 新增 `0033_workflow_templates`，模板写入使用版本校验。隔离模板逻辑 34 项、混合选区右键保存、四种携带组合、失败重试、含附件分享还原及模拟完整执行通过，编排协议 50 项、删除恢复、触发器回归及桌面构建通过；未调用付费模型、改写真实库或发布安装包。详见 [工作流卡片](dev-doc/CANVAS-WORKFLOW.md)。

> **工作流卡片阶段存档与编排助手开工（2026-09-23，源码未发布）：** 存档现有内容卡独立存图、引用生命周期恢复、生成历史、并行调度、错误定位、复制及本机 Agent DS 文本改写链路。工作流逻辑 20 项、隔离运行/调度/引用生命周期回归、桌面 TypeScript、Rust 工作流 11 项，以及 Worker 文本改写与统一处理器 16 项通过；未调用真实模型、未发布安装包或云服务。随后完成工作流助手开发版原型：复用本机 DSH，把需求转换成已有卡片、参数与连线，校验后一次落盘，默认不执行；支持结果取回、取消、定位及受保护撤回。助手隔离界面与 48 项检查、新画板初始化/拖动回归及构建通过；真实 DSH 已完成三张长图分支编排并落盘，另经原生粘贴放入人工对照流程，原卡片和执行记录保留，均未触发生图。修正卡片引用语法契约与错误定位，并提供内容卡各图片格独立来源；生成效果与长图保版尚待验收。
...
**2026-09-24 图片标注添加到画板（源码未发布）：** 图片标注器增加「添加到画板」出口，打开时冻结当前项目与画板落点，保存沿既有 annotation 入库链路保留来源及坐标元数据，再创建独立图片卡；临时项目先持久化，失败保留编辑器并复用已入库素材重试，没有可用画板时禁用。原素材库保存与临时插入创作板出口保留。隔离回归覆盖重载、失败重试不重复入库和不可用画板，尚未发布安装包。

**工作流模板与子流程（2026-09-24，源码未发布）：** 模板库为本机 SQLite 持久化的跨项目定义，文件分享采用白名单 schema v2，兼容 v1。保存入口为画板框选后右键「存为模板」，确认名称及是否携带素材/文本；文本指输入文案，操作指令始终保留。携带内容以可移植图片字节和正文保存，复用时恢复为新内容卡；未携带内容保留待填输入。模板库不再要求重新勾选步骤。实例展开为现有卡片并由触发器提供统一参数入口，保持现有调度、锁定和恢复；不新增 Runner。内部节点身份在重新配置时保持稳定，并清除下游过期结果、保留生成历史；内部定义手工修改或运行中时拒绝统一覆盖。当前为静态展开式复用，不是可折叠的动态嵌套调用；参数支持附加要求和生成比例，公开输出定位到实际内部端口。导入不会自动执行或复制账号配置；尚无在线模板市场/同步。

**工作流删除与保存恢复（2026-09-23，源码未发布）：** 删除生成等工作流卡片时，同时清理失去输入的隐藏内容 writer，保留内容卡已存文字和图片。曾因空 writer 违反后端校验，使保存队列持续报「工作流数据无效」；重试保存现能修复该残留并保存当前未落盘编辑，无须重新加载。严格保存校验回归通过，实际开发画板重试已恢复保存；不改变执行和计费链路。

**生成卡片产物连线（2026-09-23，源码未发布）：** 工作流会话卡隐藏后，画板上仍可见的当次及历史生成图片，其产物线直接连回所属生成卡片；按会话执行身份与图片实例匹配。收纳进内容卡片时保留内容输入线，不绘制隐藏图片的悬空线；断开后独立图片及其产物线恢复。不新增或改写执行依赖。

**新画板工作流初始化与拖放修复（2026-09-23，源码未发布）：** 未落库空白画板不再提前请求 `project_canvas_get`，工作流从空文档初始化；持久项目待画板 ensure 完成后再读。首次新增先创建项目记录再保存卡片，保持空白不落库约定。工作流工具栏支持拖到指定位置新建，原点击与标题栏移动保留。严格模拟缺失画板的初始化/拖放/持久化回归、原助手/Agent/工作流/草稿回归与构建通过；未改写用户库或发布。详见 [工作流卡片](dev-doc/CANVAS-WORKFLOW.md)。

**工作流编排助手范围（2026-09-23，开发版原型，未发布）：** 新增独立“工作流助手”卡片，根据需求组合现有指令、文本 Agent、生成、技能、视觉规范及触发器；使用现有 Agent 通道，不新增领域 Runner。第一版使用开发版本机 DSH，模型只提交受校验的结构化编排，桌面负责实例化真实可编辑卡片与连线；只新建流程，使用接入助手的来源，不修改已有流程。需求框与生成卡共用 `@` 输入引用，可在句中指定文本、图片的用途；发送时转换为来源别名，显式引用的文本完整传入，断开引用或超过上下文上限明确阻断。新编排使用 v2 声明式方案与 proposal/反馈/commit 协议：连线及触发器由应用编译，来源用途与交付清单进行逐份覆盖、共享依赖和死分支校验，最多三版修订；只有与已校验内容一致的最终提交可以落卡。搭建与执行分开，默认不自动提交生图/反推等模型任务。请求身份及整批落卡持久化，支持取消、重载取回和撤回未使用结果；正式构建隐藏入口。现有文本 Agent 保留文本处理职责；不扩大为整个画板助手。

**内容引用生命周期修复（2026-09-23，源码未发布）：** 复核真实生成卡发现旧单元格已不存在，但旧输入线与新增有效线同时残留，`@` 菜单又将已删除格误当「等待上游内容」，导致用户重插引用仍选回旧 ID。现按来源身份统一区分存在（可为空）、待创建的自动产物及已删除；已删除格在菜单中禁选并明确标注。重连、画板来源变化、载入和执行前协调输入线与全部提示词引用：只用已经明确接入、同卡唯一且未被其他引用占用的有效线替换旧来源，同时去重输入；没有替代连线不猜选其他格。空格仍可预引入，上游正常执行，读取时为空才失败。真实界面回归覆盖编辑/插列身份、删除/撤销、禁选旧格、重连同步、旧记录载入和生成；复制、Agent、空内容、调度、分层及文本编辑回归通过。真实库只读验证本次报错卡可明确恢复；另两张历史生成卡仅有已删除来源，须指定新连线，不能自动猜配。未改写真实库或调用模型。

**多个文本引用中的单格换线修复（2026-09-23，源码未发布）：** 实际失败工作流只读核验发现，Agent 输入线已指向新单元格，`@文本 2` 仍指向同卡旧单元格；此前仅支持唯一输入换线，导致新来源被过滤、漏掉依赖并误报空文本。载入旧记录及编辑连线时同步恢复明确对应关系：只有一个失配的活动引用，且同一内容卡只有一个尚未被其他引用占用的候选，才跟随新格；其他引用及 prompt token 保持，多个候选仍要求选择。Agent 区分未匹配连线与真正空文本。原失败卡用修复代码只读解析，两路文本均非空；新增多引用换线顺序、载入/持久化、等待新 writer 和不抢占其他来源回归通过，相关输入/调度/分层/Agent/UI 与构建通过；未写改真实数据库或提交真实模型任务。

**下游内容引用在执行时检查（2026-09-23，源码未发布）：** 下游 `@` 引用绑定单元格/内容卡身份，空文本、空图片仍可预先引用；工作流规划只安排依赖，不因下游引用暂时无内容而拦住上游。等待本次上游写入后再读取，仍为空或引用失效时，在消费卡片执行步骤报错、不提交该卡模型任务，保留上游已完成结果；生成卡不能把空文本引用静默忽略。多个已选来源不因其中一个失效而自动合并到另一来源。新增 24 个隔离执行场景覆盖单格/整卡、文本/图片、先空后填、仍空、来源删除、失效引用及生成/Agent/指令消费者；空格 `@` 菜单、原运行/调度/分层/Agent/诊断回归、类型检查与生产构建通过，未真实付费调用或打包发布。

**单元格连线校验修复（2026-09-23，源码未发布）：** 单元格输入连接只检查连接图循环，不再误用下游运行规划；下游生成/Agent 卡中的失效 `@` 引用不会阻断图片单元格互连。连接可用图片后立即写入目标格，真正运行包含失效引用的卡片时仍报错。已复现「文本 2」错误并通过图片/文本输入回归；自连、反向环与整体输入环仍拒绝。

**内容单元格独立存图与悬停置入（2026-09-23，源码未发布）：** 拖拽单张画板图片或素材栏图片，在内容单元格内悬停 1 秒后显示就绪提示，松手写入；沿用悬停建文件夹的计时与取消规则。置入与图片端口输入使用相同存图逻辑，新图覆盖目标格旧图，无新图则保留并输出已有图片；批量输入只替换收到新图的位置。单元格独立持久化素材引用，不依赖外部图片实例或连线；悬停置入不创建隐藏连接，按移入处理：目标单元格保存成功后移除被拖拽的画板图片实例，保存失败则保留原卡；同图其他实例及中央素材文件保留。通过端口连线输入则保留画板源卡。读取图片单元格不自动重跑上游输入，只有本次流程确实送入新图时才等待写入后读取。断线或移除外部画板图片不清空已存内容。隔离浏览器覆盖悬停/释放、取消、重入、切格、缩放、素材栏置入、重载、覆盖输出、来源移除及断线保留；输入、执行顺序、并发调度回归与类型检查/生产构建通过。未打包发布。

**工作流生成历史入口（2026-09-23，源码未发布）：** 生成卡片的图片输出节点下方提供「生成历史」，列表气泡按新到旧展示每次生成会话，点击打开对应会话面板；支持空态、运行状态、点击外部或 Esc 关闭。每行展示本轮首张产物缩略图及多图数量，复用对话框悬停约 2 秒显示原图、边界避让和滚动收起逻辑；点击缩略图打开对应会话。工作流拥有的当前与历史会话不再以独立卡片/端口出现在画板，也不参与框选、吸附或适应内容；下游直接从生成卡图片出口连线。会话记录、结果图片与执行恢复保留，旧工作流重载后沿用已有归属收进历史。超出最近任务缓存的旧会话按 job 从持久化图恢复查看，不混入同线程其他运行。隔离浏览器覆盖生成串联/重跑、历史列表/面板、关闭/重载、普通会话与图片容器回归，会话快照单测、工作流运行、类型检查和生产构建通过；未调用付费模型或打包发布。本条取代旧版「会话卡显示在工作流连线中间」展示约定。

**画板 Agent DS 自动送达测试（2026-09-23，开发版）：** Agent 卡片在 dev 默认走既有本机 Agent DS 队列与会话自动送达，可选切回 Cloud DSH；不是另起临时 harness。输入仍由卡片 @ 引用决定。工作流持久化投递 ID，DSH 按消息约定向 `ds-inbox/results/<请求ID>.json` 原子回写文本或错误，卡片验证后才推进下游。自动送达失败只停在等待，队列保留；继续/重载不重复投递，停止不打断用户共享会话。正式构建保持 Cloud 通道。当前为源码与隔离测试验证，需重编开发桌面后由用户实际点击测试；无 VPS 部署。

**工作流按依赖并行调度（2026-09-23，源码未发布）：** 同一触发器下所有依赖已完成的步骤同时执行；每个步骤结束立即推进自己的下游，不等待无关慢分支。汇合节点按实际引用和内容卡单元格 producer 等齐输入，文本卡起点仍读取启动快照；等待规范确认或原任务结果只暂停相关分支。沿用模型/账号并发门控，短时画板读改写串行保护，同一原图拆分排队复用工程；不同图片拆分独立完成、导出并推进，不再等待整批拆分结束。失败后不再启动新步骤，已提交步骤退出前保留锁，诊断对应实际失败节点；停止取消所有活动任务并等待回调结束再解锁。重载恢复所有中断步骤，保留各自任务身份，不自动重新提交。调度专项覆盖并行生成/反推、快分支推进、内容格汇合、并发写入、人工确认、重载/继续、失败及停止；工作流 UI/运行、分层、生成串联、内容输入、发条、Agent、错误定位与反推缓存回归、类型检查和生产构建通过。未调用真实付费模型或打包发布。

**画板 Agent 文本改写卡片（2026-09-23，源码未发布）：** 左侧新增 Agent 卡片，以文本端口接收原文，在卡片编辑器通过 @ 列表选择引用并填写修改要求，文本端口输出完整改写结果；只连接不自动注入，引用跟随来源身份读取本次结果，支持上游未产出时预先引用；可替换原图文案、简写反推描述并减少具体细节约束。输入与指令分开冻结，复用 `bowerbird-unified-agent` / Cloud DSH、账号门控、计量代理与 checkpoint，不新增 Agent Runner，不授权图片/文件/网络工具。正常运行自动等待文本结果并推进下游，重复运行读取上游最新内容；可接内容卡片主输入追加或单元格输入替换，也可接生成卡片并通过 @ 使用。源码的 Rust 参数、云端输入校验和 Worker 文本分支需配套发布；目前仅离线/模拟验收，未真实模型调用或云端部署。

**工作流引用换线与失败标记（2026-09-23，源码未发布）：** 单一动态输入改接新来源时，@ token 跟随当前连线，即使旧卡仍存在；依赖规划和 prompt 编译统一解析，避免旧引用导致过滤上游依赖、生成提前。真实图只读验证反推→写入→生成顺序恢复，18 项逻辑及分层回归通过。错误卡片红框复用运行边框的宽度、缩放补偿和遮罩，相关错误输入线变红；关闭问题面板仍保留标记，后续运行状态替代旧错误。红框/红线交互测试及构建通过，未发布。

**工作流错误定位（2026-09-23，源码未发布）：** 工作流提供持续显示的问题面板，区分启动检查与步骤执行失败，展示卡片/操作、步骤、输入或 @ 标签、来源单元格行列、原因及处理建议；可定位出错卡片/仍存在的来源、复制诊断信息。执行诊断随步骤保存，重载恢复，卡片可重新打开问题；等待状态不按失败展示，下游未执行注明上游失败。执行错误不再误用「重试保存」。诊断不主动附带 prompt 正文、图片或凭据，保留错误码、卡片/运行身份及时间。隔离浏览器覆盖启动归因、执行引用、定位、复制、恢复、来源空值与等待状态，分层/并行回归及构建通过；未付费调用或发布。

**工作流批量复制粘贴（2026-09-23，源码未发布）：** 画板混合框选后用 Ctrl/Cmd+C、Ctrl/Cmd+V 或右键复制/粘贴，一起复刻工作流卡片、内容卡片、图片实例、文件夹及选中分区成员。保留参数、发条、相对位置和内容，选区内部连接与提示词引用全部绑定新卡片；同画板未选中的外部输入继续引用原来源，外部发条连接不复制。新副本清空执行结果、会话归属和运行缓存，不自动执行；会话及执行历史不纳入复制。素材仍引用中央库文件，图片实例获得新身份。粘贴避开已有卡片并选中新副本，可整体撤销；部分保存失败重试沿用同一组身份。跨项目粘贴保留素材与内部连接，移除无法跨项目使用的外部节点连接；需要重新绑定的行内引用保留并在执行时明确报错。隔离 Chromium 验证真实快捷键、连接重映射、副本独立执行、撤销/重复粘贴、重载及保存失败重试；跨项目/文件夹通过克隆逻辑验证。未真实调用模型或打包发布。

**素材连线绑定画板实例（2026-09-23，源码未发布）：** 从画板图片端口拖线同时保存素材 ID 与具体实例 `assetNodeId`，连线、提示词引用身份及容器去重按该实例处理；同素材多次摆放不再取第一个匹配项。移动/重载保持来源，移除实例后不跳到另一副本，执行前提示重新连接；撤销恢复原实例即可恢复连线。旧素材 ID 连线因缺少来源实例信息继续兼容，重新连接后记录实例。隔离浏览器验证双副本真实接线、拖动、重载、移除/恢复、运行预检及引用身份；未付费调用或发布。

**历史会话归还图片连线（2026-09-22，源码未发布）：** 重跑清空/替换当前会话后，旧会话恢复「会话 → 本轮独立图片」原生结果连线和图片展示；容器残留旧图不会继续隐藏历史结果。图片收起须同时满足存在有效图片容器连接、对应目标格已接住该图；显式断开也恢复展示，容器内容保持。新会话仍可接续原下游并独立收起自己的容器图片。隔离浏览器覆盖重跑启动/新会话替换、旧图残留、结果连线恢复、当前图片隔离、手动断开与重载，生成串联/图片容器回归通过；未付费调用或发布。

**拆分单出口与图片容器（2026-09-22，源码未发布，取代分层产物文件夹）：** 指令卡分层仅保留一个「产物」图片出口。已连接图片容器时写入目标单元格，保留其他内容；没有图片容器连接时自动新建一个容器接住全部图层，即使另有其他下游也会创建。自动产物卡按接近方阵、优先增列排布，每个素材独占单元格，旧多图单格在重跑时升级。重跑复用自动容器及单元格身份、替换本次图片；删除输出内容卡或目标单元格后再次运行会按本次写入范围清理失效连接（含同表残留的文本/图片 writer），分层没有有效目标时自动重建产物卡，不再误报源图片缺失。 删除分层产物卡后，可确认来源的整组图片引用在清理 writer 前转接原分层 image 输出，同步更新下游输入及生成卡 @ 引用；兼容 resultNodeIds 已替换的历史自动产物 ID，避免下游继续读取旧卡。单张多格引用不擅自改为整组。 生成卡按可复用流程延迟取值：旧容器已删除但该类型仅剩一个有效输入连接时，@ token 跟随当前连接，prompt 不变；启动前容器为空不阻断上游，执行到生成卡再读本轮结果。真实画板只读隔离预检/排序及同 prompt 两次换图回归通过。不再逐层创建画板图片节点或文件夹；既有历史文件夹不删除。旧逐层端口连线及提示词引用载入时归并到聚合出口。自动容器可中继本轮产物，单卡运行同样填充显式容器并纳入写入保护。隔离浏览器覆盖创建/落位/重跑/显式填入/中继/旧线兼容/重载，并行拆分、运行、图片容器与生成串联回归、逻辑 17 项、类型检查和构建通过；未付费调用或发布。

**会话图片输出与容器去重（2026-09-22，源码未发布）：** 会话卡右侧悬浮显示图片输出节点，可接图片容器或其他图片输入。当前工作流会话的连线仍绑定生成卡，重跑随新会话接替；普通/历史会话按其 job/turn 读取该轮全部图片，不混入后续轮次。会话结果已存在于可见图片容器时，画板收起对应独立结果卡片、结果连线及其选区/端口，保留原生记录和素材；清空或移除容器后恢复独立展示。多图、精确轮次下游、当前生成卡绑定、重载与清空恢复的隔离浏览器测试，以及图片容器/生成串联回归和类型检查通过；未发布或调用付费模型。

**统一内容卡片（2026-09-22，源码未发布）：** 图片容器与文本卡片合并为工具栏单一「内容卡片」，默认文本，新行列默认文本；单元格按输入切换图片/文本，支持同表混排和图片点击放大。整体文本输入追加第一列新行；主图片输入每张图片各占一格，首批按接近方阵、优先增列排布（列数为图片数平方根向上取整，逐行填入），空白新卡直接使用第一格，已有内容保留。重跑复用位置，增加时补充单元格、减少时保留未收到新图位置的原图；单格输入替换。整体和单格均有绿色图片输入，原紫色入口保留兼容；加列按钮放在表格底部，加行按钮放在表格左侧，恢复悬停对应行列边界才显示加号和引导线，选中卡片不常显；端口优先接收点击，顶部按最新要求恢复加高前的 30px 标题栏，移除新增头部留白，主节点回到顶部两侧；左右留白仍为 36.8px。整体分别提供图片和文本汇总输出，保留原卡片/单元格 ID 与动态 `@` 引用。旧图片容器兼容读取，写入后归一；反推结果为图片与提示词两列。混合图文、整体双输出驱动生成、旧卡重载、预览、原表格/工作流回归及原生保存/依赖校验通过。包含原生 schema 扩展，需更新桌面程序，未打包发布。详见 [工作流契约](dev-doc/CANVAS-WORKFLOW.md)。

**图片容器卡片（2026-09-22，已合并为内容卡片，源码未发布）：** 左侧画板工具新增图片容器，复用原生表格的行列增删、尺寸调整和稳定单元格身份。主输入每条连接在第一列新增一行，单元格输入替换该格；每格可容纳同次输入的多张图片，可清空；点击图片打开大图预览，可切换容器内图片与缩放。整体输出按行列顺序去重汇总，单元格输出仅含该格图片，节点悬浮显示、输入右键断开保留内容。可中继工作流图片并作为生成卡 `@` 候选；整体输出依赖所有写入步骤，启动冻结外部输入、实时读取本次产物，拒绝文本输入与循环。以 note_type=images 保存，图片引用纳入项目素材关联，不复制文件。隔离浏览器、工作流回归与原生数据/依赖测试覆盖；涉及 Rust，需更新桌面程序，未打包发布或调用付费模型。

**生成卡图片输出与会话中继（2026-09-22，源码未发布，取代专用会话出口）：** 生成卡仅保留「图片」输出，逻辑上连接下一输入，视觉上由本次会话卡居中衔接。重跑立即废弃执行范围内旧结果，新会话接替当前连线；旧会话留在画板作历史。下游 `@生成结果` 引用来源端口，每次等待本轮全部图片入库，不绑定旧图片 ID；失败停止下游，恢复按原轮次取图，手动续轮不自动推进。共享只读与并发规则保持。串联/重跑/失败/恢复/连线重载及既有回归、原生落位、类型检查与构建通过；未真实付费调用或发布。详见 [工作流契约](dev-doc/CANVAS-WORKFLOW.md)。

**工作流共享只读依赖（2026-09-22，源码未发布）：** 多张生成卡引用同一已确认视觉规范等既有上游结果，允许并发读取；仅在执行会改写其他运行使用的卡片/原生文本表时拒绝启动。来源参数、输入、删除和重新执行继续受修改保护；从来源输出接到新下游不属于修改来源，允许进行。启动预占、恢复与 Rust 存储校验统一区分读/写范围。详见 [工作流契约](dev-doc/CANVAS-WORKFLOW.md)。

**反推自动文本卡接主输入覆盖修复（2026-09-22，源码未发布）：** 主输入连线本身正确新增第一列单元格；根因为自动结果归属仍在，反推重跑整表重建删除新增行。现在显式接入文本卡主输入或单元格输入时解除该卡的自动整表重建归属，只由输入写入目标格；已有双重归属的保存记录载入时同样清理，其他内容及单元格身份保持。回归先复现失败，再验证主输入新增、原文保留、重跑只更新该行及旧记录重载；文本输入/触发器浏览器回归、类型检查与生产构建通过，未发布。

**反推统一提示词输出与文本落卡（2026-09-22，源码未发布）：** 反推不再生成维度输出端口，只保留「提示词」并汇总全部维度。已连接文本卡整体/单元格输入时写入对应单元格，保留其他格与样式；没有文本卡连接时自动建立「素材 / 提示词」文本卡，单图也建立，多图沿用每素材一行。自动结果卡重跑更新原卡并保留单元格身份，删除后重建；旧维度连线及生成卡引用在载入时归并到提示词端口。模拟执行覆盖单图新建/重跑、显式文本连接不重复建卡、单卡执行写入及下游中继、旧连线兼容；原反推缓存/工作流逻辑回归、类型检查与构建通过。未真实付费调用或发布。

**反推默认复用与覆盖开关（2026-09-22，源码未发布）：** 指令卡反推新增默认未勾选的「覆盖已有反推」；未勾选时读取各输入图最新 caption 的可用维度（兼容旧 text 正文），已有则复用、缺失才并行反推，全部命中时无需可用引擎。勾选后沿用原反推接口重新处理全部输入，新记录成为当前结果，保留历史。蓝色下划线「已有反推」悬浮按图展示当前数据及缺失提示，开关保存到工作流并在运行时锁定。隔离浏览器覆盖混合输入、离线复用、覆盖、预览刷新与持久化；工作流运行/UI 回归及类型检查/构建通过，未真实付费调用或发布。

**同一工作流并行拆分（2026-09-22，源码未发布）：** 触发器下游输入已就绪、原图不同的分层指令卡同时提交/轮询拆分，各卡预先保存独立请求键。相同原图依次执行并复用工程；有数据依赖的卡等待上游产物。同批拆分完成后按原顺序导出、落位和推进下游，避免产物位置竞争；失败停止本批后续处理，已提交云任务保留原键与本地 pending 供取回，不自动重提。悬浮触发器展示多个活动步骤与各自阶段。隔离 Chromium 并发/同图/依赖/失败/停止及原工作流回归、类型检查与生产构建通过；未真实付费调用或打包发布。

**生成卡显式引用输入（2026-09-22，源码未发布）：** 生成卡文本框按 `@` 从纵向气泡列表选择收到的文字或图片，插入可保存的行内引用。端口内容不覆盖/自动追加到文本框；提交以文本框为准，展开选中的文字并仅发送已引用的图片，文本单元格中的图片索引同步映射。图片引用绑定来源容器（工作流输出、文件夹、图片容器整体或稳定单元格），不绑定当时的图片 ID；来源换图后提示词不变，下次运行读取当前图片，缩略图同步更新，旧动态引用自动兼容。来源重排不改变引用身份，断线或空来源报错，删除引用则不再参与生成与上游依赖。旧生成卡需手动引入原先自动拼接的输入。详见 [工作流契约](dev-doc/CANVAS-WORKFLOW.md)。

**分层末端直接展示素材（2026-09-22，源码未发布）：** 指令卡分层输出没有连接下游时，完成后自动展开「分层产物」并将视野定位到每张图层素材；仍保留文件夹归组和重跑去重。已有下游时继续沿工作流执行，不强制定位产物。隔离浏览器、执行回归和构建通过，未调用真实付费拆分或发布安装包。

**独立触发器卡片（2026-09-22，源码未发布）：** 统一使用左侧发条按钮：拖到卡片上吸附为发条，放在空白处创建开关卡片；金色触发线可直接吸附工作流卡片或原生文本卡本体，在原发条位置显示半齿轮，右键齿轮断开，无需输入节点；一对多启动下游，信号与文本/图片数据分开，按数据依赖排序且出错停止。原吸附发条与各卡独立执行保留，连线可保存、重载和断开。隔离浏览器、Rust 工作流校验及类型检查/构建通过，未付费调用或打包发布。详见 [工作流契约](dev-doc/CANVAS-WORKFLOW.md)。

**文本卡发条起点（2026-09-22，源码未发布）：** 发条可吸附原生文本卡，触发读取启动时各已连线单元格的当前正文与图片引用并继续执行下游；不重跑该文本卡的反推/写入来源，普通上游触发的执行关系不变。支持重载保留、运行进度和右键取下；合成浏览器、运行回归与 Rust 契约验证通过，未真实付费调用或打包发布。详见 [工作流契约](dev-doc/CANVAS-WORKFLOW.md)。

**画板运行边框（2026-09-22，源码未发布）：** 生成会话、Agent 会话及正在执行的工作流步骤显示绿色高亮流动边框，屏幕厚度固定 4 CSS 像素（原 2 像素的两倍），通过画板缩放反向补偿保持粗细；生成优先读取实时 job 状态，工作流同时检查流程与步骤运行状态，等待上游/人工确认和终态不显示跑马灯。边框不拦截点击、不覆盖正文，保留选中高亮；减少动态效果时改为静态边框。合成浏览器验证状态切换、实际动画、选中与减少动态效果，浅/深色截图已检查，追加真实缩放按钮 10%–190% 的三类卡片屏幕厚度验证通过；未发布安装包。

**文本卡整体与单元格输入（2026-09-22，源码未发布）：** 整体输入每条连接在第一列新增一行，重跑更新同一行；单元格输入写入或替换该格。文本逐字保留，图片保存 `@图片N` 索引及真实素材引用、展示缩略图与悬浮预览，文本输出继续携带参考图。输入纳入工作流排序、冻结、循环检测及错误停止，右键断线保留内容；来源已有值时立即写入，不自动启动下游模型。隔离浏览器、既有工作流回归、Rust 持久化/依赖校验和类型检查/构建通过，未真实付费调用或发布。详见 [工作流契约](dev-doc/CANVAS-WORKFLOW.md)。

**Agent DS = 投递本机 DSH 队列并自动唤醒活会话的前端，不自带 harness（2026-09-22，源码未发布）：** Agent DS 的职责只有两件——从素材库方便地把图带出来、让用户看清实际投递原文；真正干活的是用户自己开着的本机 DSH 会话（全量工具面 + 既有上下文），回复**不回流创作板**。Rust `agent_ds_chat`（debug-only）校验后走两条腿：① 原子写 `.agent-z/ds-inbox/pending/<毫秒>-<ulid>.json`（payload v1：`message` 原文 / `text` 注入视觉规范后的实际投递正文 / `images[{path,name}]` / `delivery?` 送达结果）；② best-effort 自动送达——`POST http://127.0.0.1:3080/api/session.prompt`（`mode:"queue"`，基址可用 `BOWERBIRD_DSH_WEB_URL` 覆盖）把同一内容投成该会话的一轮 user 消息，目标会话由 `session.list` 挑（排除 `origin:"subagent"` 与空白、`cwd` 命中本仓库、取最近更新），送达失败只降级为「留在队列等人工 drain」。**文件始终是权威**；消费方读完把文件搬入同目录 `archive/` 留档，参考图按 `images[].path`（+素材名）取用；**看图受会话模型限制**：纯文本模型（本机现为 `deepseek-v4-flash`）不能用 `read_image`，像素级理解须改走声明了 image 输入的模型，或经 `.agent-z/rpc` 调 Bowerbird `understand_asset`（方舟 Vision）。该路径不占生成 job/ratio/provider/entitlement、不经 DeepSeek key、不依赖 `apps/cloud/.env` 或 spike profile；`.agent-z/` 保持 gitignore，与 Agent Z/G 的 `inbox`/`rpc` 分属不同子目录。**禁止**向活 DSH 会话的 `session.jsonl.zstd` 追加事件（内存是唯一权威，外部追加会破坏 zstd 帧与 seq 连续性）；DSH web 的 `/api` 是回环可达性策略、**不是鉴权层**，不得绑到非回环地址。契约、旧路径去向与真注入红线见 [通用 Harness](dev-doc/UNIFIED-AGENT-HARNESS-PLAN.md)。

**工作流卡片统一选区（2026-09-21，源码未发布）：** 指令、生成、技能和视觉规范卡片接入画板混合框选、Ctrl/Cmd 增减选择、整体拖动、吸附辅助线及右键「整理」；移动/整理支持撤销，取消拖动恢复坐标，重开保持位置。几何变更仅保存位置，不重置参数、连接、产物或运行状态，运行中的卡片仍锁定。100%/50% 缩放的合成浏览器测试、原工作流/吸附 UI、类型检查与构建通过；尚未原生完整验收或发布安装包。详见 [工作流契约](dev-doc/CANVAS-WORKFLOW.md)。

**分层产物文件夹与去重（2026-09-21，源码未发布）：** 用户确认范围为分层后送入反推的图片。工作流产物在画板「分层产物」文件夹展示，产物端口连到该文件夹；相同原图/工程的重跑复用素材、文件夹与成员，新产物默认仅画板可见，不污染中央素材库。工程改变的新版本保留在同一文件夹，手动分层导出维持原行为；既有重复项不自动删除。原生去重/隐藏/版本测试、模拟链路及类型检查/构建通过，含后端修改，未发布。

**分层复用与工作流文本中继（2026-09-21，源码未发布）：** 分层卡先读取原图已有完整工程，有则本地复用、无则拆分，不因发条重跑重复提交；未完成任务/打开的编辑工程沿用原保护。反推产出的文本卡作为执行依赖，文本单元格连到生成后可随同一起点继续运行；重跑更新原文本卡并保留单元格连接，下游读取本次更新内容，普通外部文本仍冻结启动时正文。离线复用、完整模拟链路、依赖/循环、Rust 校验与类型检查/构建通过，未真实付费串联或发布。

**画板文件夹工作流输出（2026-09-21，源码未发布）：** 文件夹悬浮显示图片输出端口，可将多张图片连接视觉规范等卡片；按文件夹身份持久化，启动时读取当前成员顺序、去重并排除视频，固定本次输入，成员变化下次生效。展开/折叠跟随边缘，空组/删除在提交前明确报错。浏览器、工作流/视觉规范模拟、Rust 校验及类型检查/构建通过，未真实模型串联或发布。

**分层直跑与会话输出（2026-09-21，源码未发布）：** 工作流分层卡运行等同「开始拆分」，直接提交并取回结果，不再弹面板或等待手动开始；完整工程落盘后在原图显示 Layers 角标，右键原图可进入分层编辑。「产物」输出仍包含全部图层。生成卡输出改为专用「会话」类型并自动连接对应会话卡片；旧图片输出连线提示从会话结果图片重新接入。拆分沿用既有计费/报价/图片校验、保存前不提交与原键恢复规则。追加修复单层导出丢失底图结构导致校验失败；反推接收多图后并行处理（独立任务与取消句柄），结果按输入顺序汇总，输出固定「素材 / 提示词」两列的原生文本卡（每素材一行，另加表头），保留聚合/维度输出及单元格端口。发条沿杆轴旋转，悬浮可查看步骤与实际阶段。逻辑、模拟运行、浏览器、Rust 分层、类型检查及构建通过，未真实付费串联或发布。

**画板工作流局部锁定与并发（2026-09-21，源码，未发布）：** 同画板允许多个无卡片冲突的工作流同时运行；仅锁定每次运行的执行卡片及依赖的上游工作流卡片，无关卡片允许新增、编辑、连接、拖动和删除。每次运行独立停止/继续/恢复，运行开始预占卡片，迟到结果不覆盖停止后的编辑；文本单元格输入按启动时正文冻结。模型服务和账号原有并发限制仍生效。SQLite `0032` 扩展项目删除保护，兼容旧单次运行记录；并发模拟、浏览器交互、原生工作流专项、类型检查及构建通过，真实付费串联与安装包未验收。详见 [工作流契约](dev-doc/CANVAS-WORKFLOW.md)。

**视觉规范卡片（2026-09-21，源码实现，未发布）：** 在画板工作流增加图片/文本输入、专用「视觉规范」输出；支持图片、纯文字或组合提炼，亦可选用已有规范。沿用独立规范 draft→用户确认→confirmed 版本与冻结 capsule；首次提炼后暂停下游，保存后继续。生成/技能卡显式连接同类型输入，不改全局规范选择；输入不变重跑复用版本，改变来源/要求或显式重新提炼才创建新版本。规范不属于项目，来源快照同时保留原图观察与文字要求。Cloud 原提炼服务扩展 schema 2，顺序为「先更新 Worker 和 workflow 提示词，再开放 Edge capabilities，最后发布桌面」；2026-09-22 已完成前两步（VPS Worker 覆盖层镜像 + `visual-profile` v15 capabilities），**桌面发布与真实模型串联仍未做**。契约见 [CANVAS-WORKFLOW.md](dev-doc/CANVAS-WORKFLOW.md)。

**画板工作流交互修正（2026-09-21，源码未发布）：** 发条仅标记整条流程的运行起点，各卡可独立执行；反推固定提供聚合「提示词」端口，分层固定提供汇集所有图层的「产物」端口。文本卡每个单元格可输出文本，图片/文本端口悬浮显示，单元格按稳定身份绑定。生成会话锚定对应生成卡并从输出端口连线，避免普通创作自动落位干扰；执行错误终止工作流并解锁编辑，人工确认仍正常暂停。细节及验证见 [CANVAS-WORKFLOW.md](dev-doc/CANVAS-WORKFLOW.md)。

**画板工作流（2026-09-21，首版源码实现，未发布）：** 按用户确认在现有画板左侧新增指令卡（反推维度 / 复用生成提示词与参考图 / 分层图片）、生成卡（文本与图片输入、独立指令、悬浮模型与比例）、技能卡及可吸附的发条起点。工作流定义包含步骤、类型化输入输出、参数/素材绑定与执行状态；可保存、修改并重跑。沿用现有生成、反推、分层和统一 Agent，不另起模型运行链路；人工分层与技能审批暂停后继续。可编辑工作流存 SQLite `0031`，与历史溯源分开；运行固定项目/账号，恢复不自动重提未知付费任务。首版尚不含跨项目模板库、混合框选/分组/撤销、生成富文本/视频/多轮功能；技能保留既有测试账号门控。实现与验收边界见 [CANVAS-WORKFLOW.md](dev-doc/CANVAS-WORKFLOW.md)。

**2026-09-20 画板新卡脑图式落位、生成后聚焦、视频卡独立样式与文本卡片表格增强：** ①新生成卡片（普通生成指令卡/Agent 卡）按引用图位置脑图式落位：后端 `place_prompt_near_inputs_tx` 派生的临时位置（参考图右侧 +72、顶部对齐、遇既有内容下推）前端原样保留，不再锚定到可视区左上角（mac 26.9.2001 的左上角方案经用户确认否决，2026-09-20 Windows 端改回）；仅当临时位置被占时才在可视区内就近避让。生成后恢复聚焦：视图缩放适配（只缩小不放大）并居中该卡（`canvasViewForNewCard(…, true)`）；参考图仍随卡排在卡下方、输出图在指令卡右侧。`canvasPlacementForNewCard` 契约改为「无碰撞即保留临时位置」，引用落位 UI 回归同步改判脑图落位与居中聚焦。②视频卡片与图片卡样式区分：asset 节点按 storePath 判视频加 `is-video`，紫色 2px 描边、深色底、专属 ▶ 徽标与名称栏渐变，素材组内视频缩略图同紫调（media 回归断言类与徽标）。③文本卡表格整表复制为 CSV（RFC 4180 转义），按钮并入原行距工具组（组名改「文本工具」，气泡不显示）。④表格行高/列宽拖拽自由调节：note payload 新增可选 `column_widths`/`row_heights` 正数权重（schema 仍为 1，Rust 契约校验维度一致且为正、非法拒收），插删行列同步维护权重，边界手柄与插删控件按权重定位且两轴手柄错位避让命中冲突，Escape 取消拖拽不落盘。⑤文本卡标题可编辑（T 图标右侧输入框，payload 可选 `title`，重开保留；气泡不显示标题）。验证：画板纯逻辑 127/127（新增 CSV/权重两测）、reference/notes/bubble/section/marquee/media/transparency/snap/layers/draft UI 回归、Rust 379 passed / 6 ignored（新增 note 契约一测）、TypeScript 与 production build 通过；`canvas-arrangement-ui` 与 `canvas-selection-to-board-ui` 在改动前 HEAD 即因「整理」菜单等待失败，属既有问题未处理。
...
**2026-09-18 入门路线暂时开放范围（已纳入 Windows 26.9.1803）：** 仅设计师路线可进入；市场运营、视频编导身份卡暂时禁用，卡片小字显示「正在准备中」，新建与恢复入口均不可点击。保留两条路线实现及已有进度，后续可恢复开放。

**2026-09-18 设计师引导文案与 7.1（已纳入 26.9.1802 本地签名包）：** 按最新文案调整第 3–9 步及换行，第六步仅正文，移除指定登录框说明和通用操作提示。第七步激活对话框后进入 7.1，介绍自然语言与 AI 工坊辅助生成，用户手动下一步才进入第八步；沿用原步骤索引与持久化进度，暂停/重启可继续，继续时恢复创作模式以打开维度环。第十步按钮改为「完成设计师路线引导」，后续仍为既有完成/扫码弹窗。详见 [ONBOARDING.md](dev-doc/ONBOARDING.md)。

**2026-09-18 桌面自动更新：** 采用 Tauri 官方 updater、固定平台公钥和不可变签名包；官网统一检查入口，按平台发布版本清单。共享客户端源码已增加每次启动静默检查，新版弹窗由用户选择下载、安装或本次跳过；无更新或检查失败不弹窗，设置保留手动检查。该启动行为已纳入 Windows 26.9.1803；Mac 26.9.1901 已发布，发布说明包含此能力，原生启动提醒仍待验收。更新专项细节集中维护于 [DESKTOP-UPDATES.md](dev-doc/DESKTOP-UPDATES.md)，包括 Windows 清登录边界、Mac 独立签名及 R2 下载、发布回滚与双端共识；本页只保留项目摘要。
...
### 编排修订不能因草稿改写终止，JSON 键顺序不能充当来源身份（2026-09-23）

新协议真实运行中，模型改写 revision 1 后再提交 revision 2，接收器却已因同版本改写进入 failed。现将这类草稿错误作为拒绝反馈继续修订，保留不可变校验记录与最终提交校验，并提供旧失败请求取回入口。真实存档只读重放同时发现：Rust serde_json 排序对象字段后，原始 JSON 字符串比较会误判未变化的需求/来源；现在规范化对象键、保留数组顺序，兼容旧 contextKey。独立卡片错误合并反馈，临时读取/反馈竞争在原有轮询预算内重试。协议50项回归及真实存档隔离恢复链验证通过；真实第二版仍含无用且语法错误的视觉规范卡，不能据隔离修订样本宣称用户画板已落卡。详见 [工作流恢复](dev-doc/CANVAS-WORKFLOW.md)。

### 编排结果文件的原子写入不等于最终提交（2026-09-23）

工作流助手曾先发布方案文件、后自检补齐引用；桌面读到第一版后永久 failed，导致文件已经修正而卡片仍显示旧错误。现将新编排分为 proposal → 桌面编译校验/持久化并反馈 → 同版本同 SHA-256 的 commit，最多三版草稿；提示词引用作为生成/文本 Agent 图文连线的唯一声明，触发器由应用编译。来源用途与交付清单用于检查逐页覆盖、共享产品/文案依赖和无用分支。校验负责结构与声明一致性，不能保证模型理解或最终画面。旧 failed 请求提供原请求取回入口；仍为开发版，协议见 [工作流文档](dev-doc/CANVAS-WORKFLOW.md)。

### DSH 默认模型声明为纯文本，投递过来的参考图不能直接看（2026-09-22）

Agent DS 改成「投递 DSH 队列」后，第一版设计假设消费方（本机 DSH 会话）能用 `read_image` 直接读 payload 里的 `images[].path`。实测被拒：`read_image` 按**当前会话模型的声明**放行（DSH checkout `packages/fs/tool-fs/src/read-image.ts`：`inputModalities` 不含 `image` 即拒绝），而 DeepSeek 系在 `packages/llm/llm-deepseek/src/adapter.ts` 里被声明为 `inputModalities: ['text']`（本机会话模型 `deepseek-v4-flash` 即命中）。同一道声明也拦住 `/api` 上的图片上传（`host/apiproxy/src/api-proxy.ts`），所以「传图给我看」在文本模型上不是传输问题、是模型能力声明问题。

- **解决/绕过**：三条都不需要改 payload 契约——① 把会话切到声明了 `image` 输入的模型；② 视觉外挂给 Bowerbird：由消费方写 `.agent-z/rpc/<id>.req.json`（`{"id","tool":"understand_asset","args":{"image_path","prompt?"}}`）等 `<id>.resp.json`，走桌面端既有方舟 Vision 反推链路拿文字回来（需要桌面 app 运行中）；③ 需要"看图"的步骤交给子代理/工作流里能看图的模型执行。
- **教训**：跨 harness 传图的契约里，路径与素材名是必须的（三条路都要），但"能不能看见"由接收端模型声明决定——设计时先确认接收端能力，别把 `read_image` 的存在当成可用。

- **2026-09-21 文本结果卡输入无法断开（源码未发布）**：此前只绘制反推到文本卡的路径，未提供输入端口操作。补充悬浮紫色输入点，点击/右键断开 `resultNodeIds` 关联，保留文本及其下游单元格连接；运行中禁止断开，重跑不重新绑定已脱离文本。浏览器回归及类型检查/构建通过。

- **2026-09-21 视觉规范卡 cloud 未知 action（源码未发布）**：工作流图片输入也无条件请求新增 `capabilities`，并把空 requirements 当 schema 2，旧云尚未部署该 action 时直接失败。纯图片改走既有 schema 1 提炼；非空文字才在观察前检查新版能力，旧云明确提示，不丢弃用户文字。Rust 兼容测试、图片/混合输入模拟、类型检查/构建通过；需重启更新后的桌面后端生效，未云部署或真实调用。

- **2026-09-21 发条流程被未产出的视觉规范旁支阻断（源码未发布）**：只读检查实际工作流确认，文本连线已纳入链路，但生成卡另接一张空输出视觉规范；旧调度仅沿起点向下遍历，因而报「起点依赖的上游卡片尚无结果」。改为补齐缺输出的必要上游，保留已有输出复用与规范人工确认，不扩展旁支其他消费者；已模拟完整文本中继及确认后继续，不修改用户保存数据。

- **2026-09-21 工作流分层预览黑底（源码未发布）**：通用 JPEG 缩略图通过 `to_rgb8()` 丢弃透明通道，工作流卡片此前优先读取该缩略图，透明像素可露出黑色；改为复用画板 `canvasAssetMediaPath`，支持透明度的格式读取原图。浏览器像素回归确认 alpha 0/128/255 与不透明彩色前景正确，类型检查通过；这不代表已检查用户具体图层像素，若非透明主体仍黑需核对对应原 PNG。
...
### Windows 构建复制误排除平台源码（2026-09-18）
