...
function serializeDoc(doc) {
  ...
  // 导入图（src 为 data: URL）作为上传参考图随请求发送；预设图仍走 referenceIds 由后端读盘。
  const referenceUploads = referenceIds
    .map((id) => assetById.get(id))
    .filter((asset) => asset && typeof asset.src === "string" && asset.src.startsWith("data:"))
    .map((asset) => ({ id: asset.id, dataUri: asset.src }));

  return { prompt: paragraphs.join("\n"), referenceIds, referenceUploads };
}
...
function renderDimensions() {
  ...
  const titles = Object.keys(asset.dimensions);
  if (!titles.length) {
    // 导入图没有反推维度，整图作为参考
    const hint = document.createElement("p");
    hint.className = "dimension-empty";
    hint.textContent = "导入图暂无反推维度，整图作为参考";
...
function updateDerivedUI() {
  const { referenceIds } = serializeDoc(view.state.doc);
  document.getElementById("editor-state").textContent = `${referenceIds.length} 张参考图 · ${view.state.doc.content.size} 个文档单位`;
  renderDimensions();
  renderCreationGraph();
}
...
// canvas 压缩到长边 1536px、JPEG q0.82；处理 EXIF 方向。无 createImageBitmap 时回退原图 dataURL。
async function fileToDataUri(file) {
  ...
  if (typeof createImageBitmap === "function") {
    ...
    if (blob && blob.size > 0) return blobToDataUri(blob);
  }
  return blobToDataUri(file);
}

// 导入图命名：取文件名去扩展名、限长；与已有名（含预设）冲突时追加序号，绝不覆盖预设。
function uniqueUploadName(rawName) {
  const base = (rawName || "").trim().replace(/\.[a-zA-Z0-9]+$/, "").slice(0, 16) || "我的素材";
  if (!assetByName.has(base)) return base;
  for (let index = 2; ; index += 1) {
    const candidate = `${base} ${index}`;
    if (!assetByName.has(candidate)) return candidate;
  }
}

function addUploadAsset(asset) {
  assetById.set(asset.id, asset);
  assetByName.set(asset.name, asset);
  const card = document.createElement("button");
  card.type = "button";
  card.className = "asset-card is-upload";
  card.dataset.id = asset.id;
  card.setAttribute("aria-label", `插入导入素材 ${asset.name}`);
  const image = document.createElement("img");
  image.src = asset.src;
...
async function importFile(file) {
  ...
  if (serializeDoc(view.state.doc).referenceIds.length >= MAX_REFERENCES) {
    document.getElementById("editor-state").textContent = `最多 ${MAX_REFERENCES} 张参考图，请先在文稿中删掉一些`;
    return;
  }
  try {
    ...
    uploadSeq += 1;
    const name = uniqueUploadName(file.name);
    addUploadAsset({ id, name, src: dataUri, dimensions: {} });
    document.getElementById("editor-state").textContent = `已导入「${name}」，点击可插入创作板`;
  } catch {
    document.getElementById("editor-state").textContent = "导入失败，请换一张图片";
  }
}

if (importButton) {
  importButton.addEventListener("mousedown", preserveEditorSelection);
  importButton.addEventListener("click", () => importInput && importInput.click());
}
if (importInput) {
...
// 拖拽到瀑布流（仅接受文件型拖拽，忽略页内图片拖拽产生的 URL 型 dataTransfer）
let dragDepth = 0;
function hasFileTransfer(event) {
  return Boolean(event.dataTransfer && Array.from(event.dataTransfer.types || []).includes("Files"));
}
assetGrid.addEventListener("dragenter", (event) => {
  if (!hasFileTransfer(event)) return;
  event.preventDefault();
  dragDepth += 1;
  assetGrid.classList.add("is-dragover");
});
assetGrid.addEventListener("dragover", (event) => {
  if (!hasFileTransfer(event)) return;
  event.preventDefault();
...
assetGrid.addEventListener("dragleave", () => {
  ...
  if (dragDepth === 0) assetGrid.classList.remove("is-dragover");
});
assetGrid.addEventListener("drop", (event) => {
  const files = event.dataTransfer && event.dataTransfer.files;
  if (!files || !files.length) return;
  event.preventDefault(); // 阻止浏览器打开图片
  dragDepth = 0;
  assetGrid.classList.remove("is-dragover");
  Array.from(files)
...
async function loadImageService() {
  ...
  try {
    ...
    const detail = payload.configured
      ? `每位访客每日可体验 ${demoDailyLimit} 次 · 支持导入自己的图作为参考`
...
generateButton.addEventListener("click", async () => {
  ...
  const { prompt, referenceIds, referenceUploads } = serializeDoc(view.state.doc);
  if (!prompt.trim()) {
  ...
  showGenerationMessage(
    ...
    `发送 ${referenceIds.length} 张参考图，请保持页面开启`,
    "loading",
  );
