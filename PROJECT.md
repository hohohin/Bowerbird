...
- [dev-doc/HTML-RENDER-PLAN.md](dev-doc/HTML-RENDER-PLAN.md) — 受限 HTML 离线排版与截图专项计划（独立无外网 Chromium renderer、整页/视口/纵向切片、Kernel `render_html` 工具、第二个官方内置 Skill，H0–H6；明确不做网页访问、自由浏览器 Agent、Vision 自检或自动修订；**H0-T1 代码侧 + H1 独立 renderer + H2 Kernel 工具/durable artifact 已完成（2026-08-27，本地测试全绿）**，migration 0044/Edge 待部署，H0 容器实机验证与 H3+ 未开始）
...
**测试基线：** 桌面 `cargo test` 现为 205 通过 + 2 ignored（Agent Z 真机 e2e，需交互桌面会话），前端 `tsc --noEmit` 与 production build 通过；agent-worker **158/158** + `tsc --noEmit`（三项均 2026-08-27 存档前实跑复核，较 08-26 视觉设定基线 191/153 多出 Ed25519 验签 / FTS 三列 / 引用计数 / 孤儿取回的新测试）；Deno Edge check、共享模块 6/6、Rust `cargo fmt --check` 与 `git diff --check` 通过。真实 Supabase/VPS/DeepSeek/方舟 Vision/Seedream 双审批修订、可信计量、原子结算、TTL/容量/账单、安全冒烟、Cloud 生图/理解成功与退款、Worker 重启故障注入基线有效；A2-T9 无 provider 生产控制面 E2E 两次通过。远端迁移现为 `0001`–`0043`（`0040` A8 观察、`0041` 视觉设定队列、`0042` Agent Run 视觉设定追踪、`0043` TTL 结算歧义修复），entitlement v18、agent-run v22、agent-worker v34 为 2026-08-25 快照版本号；2026-08-26 又部署了 entitlement（`can_use_visual_profiles`）、visual-profile / visual-profile-worker（V2）与 agent-run（V4 capsule 校验），VPS Worker 四循环在线；VPS Worker 已部署 A2-T7 确定性上下文构建/压缩与 A2-T9 有限澄清；Codex Agent 真机 E2E 已成功，随后新增 UI/Rust/Edge 三层互斥回归；真实即梦 Agent CLI 完整 E2E 按用户决定暂时跳过，未计入通过项，也不再作为发布阻塞。扩展 Node tests 10/10、官网 build 基线沿用 2026-08-13 存档。html-renderer（HTML 离线排版 H1/H2，新包）**57/57**（含 8 项本机 Chromium 实跑 e2e：切片逐像素复原 / 透明背景 / 资源闭集 / sanitizer 恶意样本 fail-closed；+3 项 worker↔renderer wire 契约镜像测试）+ `tsc --noEmit` 干净（2026-08-27）。agent-worker 基线升至 **172/172**（+14：render_html 契约/Policy 门控/HtmlRenderExecutor 恢复与 fail-closed——含 kill-after-render 内存恢复、跨进程确定性重算幂等、renderer 连杀 outcome_unknown、稳定 400 终态、跨 Run 资源拒达、echo 复核，以及 html_document role/UTF-8 下载校验）。
...
26. **Agent 对话与项目视觉设定（2026-08-13；2026-08-20 对齐首版 controlled-image-edit）**：Agent 只支持三个结构化交互节点：计划前有限澄清、计划审批/修改、结果接受/重试反馈；工具运行期间不开放聊天。澄清模型只能提议，Kernel 按问题必要性、重复、次数和 intent field 影响决定是否询问；回答编译为 `IntentPatch`，变更已批准计划时必须使旧 hash 失效并重新审批。结果反馈若产生新增/换序/额外成本，同样必须形成 revision plan 并重新审批。项目长期统一视觉采用**视觉设定**而非“品牌”模型；来源必须是用户在当前项目主动选择的专用普通文件夹，提炼输入严格限定为点击瞬间“项目成员 ∩ folder_id”的已有最新反推数据。视觉设定提炼**永不上传或读取图片、不调用视觉模型、不自动补反推、不扫描整个项目、不监听素材变化、不自动重算/提示**；数据不足就停止并请用户自行整理/反推。每次按钮点击冻结 `source_scope_hash` 并产生 draft，用户确认才把结构化规则/来源本地落盘为新版本。可选方向验证图仅用候选规则纯文生图且 `reference_assets=[]`，它验证的是文字规则可用性，不是原素材看图理解；未确认不入库、不成为设定证据。已确认 `VisualProfileCapsule` 可供直接生成、智能精修和 `bowerbird-controlled-image-edit` 读取，但任何 Agent/生成结果都无权反写视觉设定。
