...
**2026-09-15 身份实操引导（已纳入 26.9.15 本地安装包）：** 新引导替换旧五步实操，首屏身份文案与实操分支沿既定约定。设计师现为八步：主界面新建 → 导入「初始引导」文件夹 → 项目/全局素材说明 → 打开探索 → 小红书拖图采集 → 展开素材库 → 点击对话框激活创作模式 → 左键点击预置反推示例图打开维度环。第 2 步在菜单 spotlight 内框选导入文件夹，第 6 步聚光整个项目画板并单独框选素材库按钮；素材归属追加「全局素材包含项目素材」，文案按用户要求换行和加粗。第 8 步定位同项目内 preset-01 或同包已有维度的收集图，复用预置数据和真实点击，不调用反推服务；引导卡避开并高于维度环遮罩。三身份均允许逐步跳过并记录 skippedSteps，保留整体跳过/暂停/重启；已完成六步的进度从新增第 7 步继续，旧项目素材不删除。纯进度 10/10、三路线八步/跳步/维度环合成 IPC、创作模式进入回归与 TypeScript/Vite 构建通过。原生小红书/OS 拖放未重新验收，桌面已随 26.9.15 本地打包，详见 [ONBOARDING.md](dev-doc/ONBOARDING.md)。
