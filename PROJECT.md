...
- [dev-doc/ONBOARDING.md](dev-doc/ONBOARDING.md) — 身份实操引导：替换旧实操，三种身份、真实操作检测、独立项目与进度、隔离验收（2026-09-14）。
...
**2026-09-15 身份实操引导（十步已纳入 26.9.15 本次重包）：** 新引导替换旧五步实操，首屏身份文案与实操分支沿既定约定。设计师现为十步：主界面新建 → 导入「初始引导」文件夹 → 聚光素材库并说明项目/全局素材归属 → 打开探索 → 小红书拖图采集 → 展开素材库 → 点击对话框激活创作模式 → 左键点击预置反推示例图开环并自动到第 9 步 → 聚光「反推提示词」，选入后聚光对话框确认已添加 → 画板整体聚光、品牌图标与「完成本次引导」。完成后展开主左栏，气泡指向登录区并可打开既有账号面板，已登录状态也不直接关闭气泡。第 2 步框选导入文件夹，第 6 步聚光整个画板并单独框选素材库按钮；第 8–9 步仅用同项目同包现有维度，不调用反推服务；第 9 步核对实际插入的示例来源及维度名称。说明卡正文 12→16px、设计师宽度 320→416px；顶部左右箭头分别回退和跳过，「继续入门引导」放到右上角 Status 下方。探索原生网页随其他区域聚光同步顶层暗色遮罩，取消聚光恢复，不刷新或丢失网页状态。回看保存边界、不自动推进，返回首步及重启复用原项目；迟到回调核对步骤访问次数。保留整体跳过/暂停/重启与 skippedSteps；已完成六步/八步的同版进度分别从第 7/9 步继续。纯进度 12/12、三路线十步/回退/连续开环选词/对话框确认/登录气泡合成 IPC、创作模式进入回归与 TypeScript/Vite 构建通过。新增 Rust 浏览器相关 7/7 与隐藏 WebView2 合成网页验收通过（遮罩开关、刷新后恢复、表单/滚动保留、CDP 拖图与 Cookie 持久化）；真实小红书/OS 拖放未重新验收；本次已重新打包，原生网页无法点击的问题仍待定位，详见 [ONBOARDING.md](dev-doc/ONBOARDING.md)。
...
**2026-09-15 Windows 打包版本：** 继续使用两位年份的日期版本，Tauri 配置与 Cargo package/lock 同步为 26.9.15；根与前端 npm workspace 的 0.1.0 不作为安装包版本。安装包仅本地交付，旧包保留，当前 SHA-256 以上方最新复核为准。
