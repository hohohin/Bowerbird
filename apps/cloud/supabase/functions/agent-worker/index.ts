...
} from "../_shared/agent-result-artifacts.ts";
...
function assertUsageBinding(item: AgentUsageItem, toolName: string, run: RunRow): void {
  const valid = isAgentUsageToolBindingValid(item, toolName, expectedImageProvider(run));
...
async function actionArtifact(admin: SupabaseClient, body: Record<string, unknown>): Promise<Response> {
  ...
  if (role === "final_result" && !allowsMultipleFinalResults(run.skill_id)) {
...
async function actionAwaitResultFeedback(admin: SupabaseClient, body: Record<string, unknown>): Promise<Response> {
  ...
  if (!runId || !leaseId) throw new ApiError("invalid_request", "缺少 runId/leaseId");
  const run = await assertLease(admin, runId, leaseId);
  let hasAcceptedPrimary = false;
  for (const resultRoles of feedbackResultRoleGroups(run.skill_id)) {
    ...
    if (acceptsFeedbackResultCount(run.skill_id, resultRoles, resultCount)) {
      hasAcceptedPrimary = true;
      break;
    }
    if (resultCount !== 0) break;
  }
  if (!hasAcceptedPrimary) throw new ApiError("invalid_request", "等待反馈前缺少有效主产物", false, 409);
  const transitioned = await transitionRun(admin, {
    p_run_id: runId,
    p_lease_id: leaseId,
