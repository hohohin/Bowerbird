...
try {
  ...
  await page.reload();
  await page.getByRole('button', { name: '继续入门引导', exact: true }).click();
  ...
  await page.getByRole('button', { name: '先自己逛逛', exact: true }).click();
  ...
  console.log('PASS onboarding: collection create/open, retry without duplicate, collapsed sidebar, inline learning toggle; sample failure/retry, real card drop/move, real editor/reference/dimension, no auto provider, preparation, persisted resume, unrelated job, failure, generation completion without continuation, upgrade dismissal, completed status, missing project, save failure.');
} catch (error) {
  await mkdir('.tmp', {recursive:true});
