...
画布和图层列表双击尚无文字副本的前景图层时，弹窗提供「识别文字后编辑」与「创建空白文字对象」两个选项，以及取消。识别选项需要登录，沿用云端图像理解的计费与次数限制，识别后进入画布与字体编辑；空白选项可离线使用。文字候选依据拆分名称/说明判断，空白转换的确认文案为：

> 是否要把该图层转变为可编辑文字对象？注意，转变后会丢失原图样式以及文本内容，但可以选择本机字体进行替换。

其他前景元素使用：

> 此元素并未识别到有文字，是否要把该图层转变为可编辑文字对象？注意，转变后会丢失原素材，整体变为一个文本框。
