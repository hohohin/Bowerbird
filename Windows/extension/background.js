...
async function fetchAndUpload(url, pageUrl) {
  ...
  if (!response.ok) {
    return { ok: false, error: `浏览器读取图片失败：HTTP ${response.status}` };
  }
  ...
  if (bytes.byteLength === 0) {
    return { ok: false, error: "浏览器读取到的图片为空" };
  }
...
function uploadBytes(bytes, metadata) {
  return new Promise((resolve) => {
    ...
    const timer = setTimeout(
      () => finish({ ok: false, error: "桌面端接收图片超时" }),
      60000,
    );
    ...
    try {
      ...
      ws.onmessage = (event) => {
        try {
          ...
          finish({ ok: false, error: "桌面端返回了无法解析的响应" });
        }
      };
      ws.onerror = () => finish({ ok: false, error: "无法连接 Bowerbird 桌面端" });
      ws.onclose = () => {
        if (!finished) finish({ ok: false, error: "桌面端在入库完成前关闭了连接" });
      };
    } catch (error) {
