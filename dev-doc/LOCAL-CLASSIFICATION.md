...
## 2026-09-18 误标根治：切断反馈回路与无示例盲匹配

用户报告矿泉水瓶产品图被误标 #女孩 #插画 #梅花 #植物。核查用户真实库（只读 SQLite）确认这不是单图偶发：模型把马蹄莲认成梅花尚属能力问题，但「女孩」上水瓶、另一张水瓶图同时命中 猫/宠物/中秋/月饼 共 8 个标签，是管线把小模型误判放大成全库污染。全库 20 个标签全部由自动发现产生、无任何人工示例；「梅花」5 次命中全部错误，「女孩」仅 1 次正确（标签诞生图）。

根因（均为模型以外的结构问题，本轮不动模型与权重）：

1. **复用滚雪球**：发现阶段注入按引用次数降序的前八标签并提示「优先复用」，误标自动抬高标签排名、进入后续每张图的提示词，形成只放大不纠错的反馈回路。
2. **无示例盲匹配**：自动处理对没有任何人工示例的标签按每批八个做纯文本 true/false 批量判断，0.8B 模型成片打 true；发现阶段提出的已有标签名也被丢弃后交给该盲匹配补挂，两个通道都在制造假阳性。
3. **提示词顺序**：先给参考标签再要求命名，助长小模型从清单里挑名字而不看图。

修改（`apps/desktop/src-tauri/src/core/local_classification/`）：

- `mod.rs`：发现参考改为按素材 ID 确定性哈希抽样的 `discovery_sample`（同一图稳定、不同图覆盖不同标签，与引用次数彻底解耦）；新增 `ground_discovered_names`，发现阶段提出的、与已有标签同名的名称直接以 local 来源落地，不再交给盲匹配；自动处理只评估有人工示例（手动正例或排除记录）的标签，无示例标签仅在用户显式「保存并寻找匹配素材 / 寻找匹配」的单标签运行中判断；「重新扫描全部图片」（`pendingOnly: false`）升级为重建：全部启用标签计入 evaluated，不再成立的自动归属被清退，人工归属与排除记录不动。
- `runtime.rs`：发现提示词改为「每个标签都来自图中实际可见的主体、画法、材质或设计用途……已有标签仅用于统一命名：仅当含义相同时才复用」；匹配提示词要求 evidence 指出图中可见的具体依据，无法指出时必须 false。
- `data.rs`：`Label` 新增 `has_examples`（存在手动正例或排除记录），面板按标签显示「未设示例，不自动匹配」，自动处理文案同步；发现/匹配协议、JSON Schema、重复/遗漏重判与人工保护逻辑未改。

验证：本地分类回归 **21 passed / 0 failed / 4 ignored**（ignored 为需真实模型/隔离环境的既有用例），新增 `discovery_reference_samples_per_asset_not_by_usage`（抽样上限、确定性、低频标签仍会被抽到）、`discovered_existing_names_attach_directly`（同名落地与新名保留）、`label_example_flag_marks_what_automatic_matching_uses`（示例标记随人工记录变化）三项；`tsc --noEmit` 与 `scripts/local-classification-ui.test.mjs` 通过。批内重复/遗漏重判路径保留但自动管线不再触发多标签批请求。
...
- 状态与 UI：`Status` 新增 `vector_supported`/`vector_installed`；新命令 `local_classification_vector_install` / `local_classification_vector_uninstall`（与分类/下载共用 busy 闸，运行中拒绝卸载）；面板新增「向量精确匹配（jina-clip-v2）」区块（约 881 MB、镜像说明、安装后需「重新扫描全部图片」重建自动标签；已安装时提供卸载按钮，目录缺失视为已卸载、幂等，卸载后立即回退 VLM 判断，标签与示例不受影响）。
