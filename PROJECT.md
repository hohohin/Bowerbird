...
**Agent 线（dev）：** 创作板 Agent A/B 双方案（A=模型挑子句 + 确定性编译 / B=skill 审查修复）+ 评测 harness（`npm run ab:prompt`）已落地；**Agent Z（创作板 × Claude Code TUI 互通实验，dev-only Windows）**——消息投递 TUI 终端 + MCP 桥回传创作板 + RPC 反调即梦生图/反推（约定 36）；A2（VPS Worker 跑 Skill）与 A4（桌面接 agent-run）未开工，规划见 [dev-doc/AGENT-RUNTIME-PLAN.md](dev-doc/AGENT-RUNTIME-PLAN.md)。
...
**测试基线：** 桌面 `cargo test` 159 通过 + 2 ignored（Agent Z 真机 e2e，需交互桌面会话）、agent-worker 59/59、桌面 `tsc --noEmit` 干净（2026-08-20）；扩展 Node tests 10/10、官网 build、Edge Functions Deno type-check 全过（2026-08-13 存档基线）。
