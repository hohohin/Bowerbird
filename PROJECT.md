...
**9 月 18 日官网增量部署：** 已同步远端 `mac`（`8534144`）的网站更新通道，官网当前目录为 `/opt/bowerbird/website-releases/20260918-mac-update-channels`，回滚目录为 `/opt/bowerbird/website-releases/20260918-app-updater-v2`。Windows 更新及下载验证通过；Apple Silicon/Intel Mac 更新入口已上线，但对应签名包和清单仍待发布。生产环境配置保持不变，部署证据见 `.tmp/mac-website-20260918/`。

**9 月 18 日正式发布：** 用户确认将工作区原有改动与自动更新一起发布，官网现提供 `Bowerbird_26.9.18_x64-setup.exe`（83,178,026 bytes，SHA-256 `f7da5237d1d8d75cab5363769c9c3f76795fe0f725525e0046a74abd9ee5a781`）。HTTPS 更新入口、禁止缓存的版本清单、公开签名、完整安装包哈希、首页实际下载按钮及官网/后台健康检查通过；从公网下载的安装包再次使用内置公钥验签并验证篡改拒绝。该次官网发布目录 `/opt/bowerbird/website-releases/20260918-app-updater-v2`，当时回滚保留 `/opt/bowerbird/website-releases/20260914-code-admin`，旧安装包继续可下载。未执行真实应用升级，不宣称原生安装/重启/登录全链路已验收；源码已纳入本次统一存档提交。发布、回滚脚本和公网验收证据在 `.tmp/app-updater-release/`。
...
**2026-09-18 Windows 应用内更新（26.9.18 已正式发布）：** 设置 → 关于我们 → 当前版本加入检查更新、下载进度及安装并重启；使用 Tauri 官方 updater，内置固定公钥验签，下载/验签完成前不可安装，重复操作互斥，关闭设置保留当前进度。安装前检查已知生成/Agent/素材整理任务并等待画板保存；Windows passive NSIS 沿用安装清登录钩子，素材、项目及独立 CLI 历史保留。老版本需要先手动安装一次具备更新入口的版本。用户确认沿用当前官网 `bowerbird.cn` 及官网 downloads 存储；新增官网更新检查入口和签名清单构建工具，先上传不可变安装包、后原子发布清单；官网与整包已按用户确认发布，未执行真实用户安装。签名私钥仅保存在 Git 忽略的 `Windows/.signing/`，已建立当前 Windows 账号加密的本地 DPAPI 备份（同目录 `updater.key.dpapi`），后续版本复用同一把私钥；此本机备份不替代异地备份，私钥未上传官网。官网已配置 Windows x64、Apple Silicon Mac 与 Intel Mac 三个 HTTPS 清单入口；目前只有 Windows 的签名包与清单正式发布，Mac 清单尚为 404，须在 Mac 侧完成构建签名并发布后才能使用自动更新；操作及验收边界见 [Windows/README.md](Windows/README.md#应用内更新)。
