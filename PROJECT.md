...
**2026-09-17 发送后聚焦会话卡片（已纳入 26.9.17 本地安装包）：** 当前项目新增普通生成指令或 Agent 会话卡片时，在卡片文字挂载后按实际尺寸将其居中到画板可用区域，选中卡片并转移键盘焦点、同步所属线程；时间线视图自动切回画板。沿用新卡片与本次新增参考图的避让布局，只在必要时缩小视图，不自动放大。重复状态刷新与重新打开项目不再次触发定位；已有素材位置不变。此条覆盖下文“新卡片不修改画板视口”的旧约定。隔离界面验证覆盖普通生成、Agent 分批卡片、无新增参考图、刷新与重开；画板契约 123/123、TypeScript 检查通过，未调用真实模型。

**2026-09-17 对话框展开规则补充（已纳入 26.9.17 本地安装包）：** 创作模式中主对话框始终展开，编辑器失焦、点击画板或窗口失焦均不收起；退出创作模式后恢复收起，露出比例由约 20% 增至约 30%。此规则覆盖下文 9 月 16 日的收起条件；详情内续轮与草稿保留行为不变。
...
**2026-09-17 Windows 打包版本：** 继续使用两位年份的日期版本，Tauri 配置与 Cargo package/lock 同步为 26.9.17；根与前端 npm workspace 的 0.1.0 不作为安装包版本。安装包仅本地交付，旧包保留，当前 SHA-256 以上方最新复核为准。
