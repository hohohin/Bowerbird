...
| Vision base64（历史） | U1 当时的 `deepseek-v4-flash-vision-exp` 配置正确声明 image capability；11,467-byte PNG 经 ACP inline base64 识别为蓝色鸟，`end_turn`。该配置与 smoke 已退出现役 Profile，不是产品路线 |
| Vision 多轮历史（历史） | 第二轮不再附图，仍能从同一 session 回答上一张图的动物，`end_turn`。只保留为协议兼容性证据 |
...
16×16 图标的首次视觉试验把鸟误判为猫但识别出蓝色，说明低分辨率 fixture 有语义歧义；换为同仓库 128×128 图标后通过。该失败只保留为历史质量风险证据，不把“图片协议可达”混同于“任意尺寸识别可靠”。按 2026-08-30 产品决策，DeepSeek/DSH 只负责文本 Agent loop，所有图片理解与结果检查统一调用 Bowerbird 现有火山方舟 Vision；现役 Profile 已删除 DeepSeek Vision 模型声明、专用 patch 和 smoke 命令。
...
- 已完成：精确钉版与 lockfile、最小 Profile、组合配置审计、secret 隔离、ACP 生命周期实测、adapter 结构化/usage/错误 fixture、真实双轮文本 tool loop；DeepSeek Vision base64 与同 session 多轮结果仅作为已退役的历史兼容性证据。
...
- U1 后续质量项：Files API 路线仅在文本上下文确有需要时再评估；视觉不再做 DeepSeek 主模型 A/B，固定由 Tool Gateway 调用现有火山方舟 Vision。
